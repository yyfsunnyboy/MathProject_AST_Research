...
# Performance: 8.71s | Tokens: In=2212, Out=324
# Created At: 2026-01-28 15:04:38
# Fix Status: [Repaired] | Fixes: Regex=9, AST=2
...
def generate(level=1, **kwargs):
    for _safety_loop_var in range(1000):
        abs_divisor_val = random.randint(2, 12)
        abs_quotient_val = random.randint(2, 10)
        dividend_sign = safe_choice([-1, 1])
        divisor_sign = safe_choice([-1, 1])
        if dividend_sign == 1 and divisor_sign == 1:
            continue
        dividend = dividend_sign * (abs_divisor_val * abs_quotient_val)
        divisor = divisor_sign * abs_divisor_val
        result = dividend // divisor
        if divisor != 0 and dividend % divisor == 0 and (-10 <= result <= 10) and (dividend < 0 or divisor < 0):
            break
    question_math = f"{fmt_num(dividend)} {op_latex['/']} {fmt_num(divisor)}"
    question_text = f'計算 {question_math} 的值'
    return {'question_text': clean_latex_output(question_text), 'answer': str(result), 'mode': 1}