...
def generate_comparison_problem():
    ...
    question_text = "已知以下點：" + ", ".join(points_desc)
    correct_answer = str(coords[-1])
...
def generate_midpoint_problem():
    # 題型：A(x1, y1) 和 B(x2, y2) 的中點 C(x, y)
    ...
    question_text = f"已知 A({x1}, {y1}) 和 B({x2}, {y2})，其中點 C 的座標為？"
...
def generate_ascii_read_problem():
    # 題型：給定一個數字序列，找到其中的最小值
    numbers = random.sample(range(-10, 21), 5)
    
    question_text = f"已知以下數字序列 {numbers}，請找出其中的最小值？"
