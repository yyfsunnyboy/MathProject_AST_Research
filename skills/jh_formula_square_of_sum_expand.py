# skills/jh_formula_square_of_sum_expand.py
import random
...
def generate(level=1):
    ...
    # 處理 a=1 的情況，讓題目更簡潔
    if a == 1:
        question_text = f"請展開 (x + {b})²"
        # 答案: x² + 2*b*x + b²
        correct_answer = f"x^2+{2*b}x+{b**2}"
    else:
        question_text = f"請展開 ({a}x + {b})²"
        # 答案: (a²)*x² + (2*a*b)*x + b²
        correct_answer = f"{a**2}x^2+{2*a*b}x+{b**2}"

    context_string = f"使用和的平方公式 (a+b)² = a² + 2ab + b² 展開題目。"
