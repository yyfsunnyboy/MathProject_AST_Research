...
# Duration: 175.82s | RAG: 7 examples
# Created At: 2026-01-07 22:52:19
# Fix Status: [Repaired]
# ==============================================================================
...
def generate_type_1_problem():
    mantissa = float(f"{random.randint(1,9)}.{random.randint(0,9)}")
    exponent = random.randint(-5, 5)
    formatted_mantissa = str(mantissa).rstrip('0').rstrip('.') or str(mantissa)
    return f"將 {formatted_mantissa} × 10^{{{exponent}}} 乘開，結果是什麼？"
