...
# Performance: 10.31s | Tokens: In=2429, Out=383
# Created At: 2026-01-28 15:04:30
...
def generate(level=1, **kwargs):
    # 隨機生成 factor1 和 factor2 的絕對值和正負號
    factor1_abs = random.randint(1, 10)
    factor2_abs = random.randint(1, 10)
    
    factor1_sign = safe_choice([True, False])
    factor2_sign = safe_choice([True, False])
    
    # 根據正負號設置因子的值
    factor1 = factor1_abs if factor1_sign else -factor1_abs
    factor2 = factor2_abs if factor2_sign else -factor2_abs
    
    # 計算最終答案
    final_answer = factor1 * factor2
    
    # 確認答案在預期範圍內
    assert -100 <= final_answer <= 100, "Final answer out of expected range"
    
    # 格式化數學表達式
    math_expr = f"{fmt_num(factor1)} {op_latex['*']} {fmt_num(factor2)}"
    question_text = clean_latex_output(f"計算 ${math_expr}$ 的值")
    
    return {'question_text': question_text, 'answer': str(final_answer), 'mode': 1}
