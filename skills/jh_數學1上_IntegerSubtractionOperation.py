...
# Performance: 18.25s | Tokens: In=2862, Out=693
# Created At: 2026-01-28 15:04:19
...
def generate(level=1, **kwargs):
    ...
    # 隨機選擇正負號組合
    sign_config = safe_choice([('pos', 'pos'), ('pos', 'neg'), ('neg', 'pos'), ('neg', 'neg')])
    
    # 根據 sign_config 生成 n1 和 n2
    if sign_config == ('pos', 'pos'):
        n1 = random.randint(1, 200)
        n2 = random.randint(-200, -1)
    elif sign_config == ('pos', 'neg'):
        n1 = random.randint(1, 200)
        n2 = random.randint(1, 200)
    elif sign_config == ('neg', 'pos'):
        n1 = random.randint(-200, -1)
        n2 = random.randint(1, 200)
    else:  # ('neg', 'neg')
        n1 = random.randint(-200, -1)
        n2 = random.randint(-200, -1)
    ...
    # 格式化數學表達式
    display_n1 = fmt_num(n1)
    display_n2 = fmt_num(n2)
    
    if n1 < 0:
        display_n1 = f"({display_n1})"
    if n2 < 0:
        display_n2 = f"({display_n2})"
    
    question_str = f"{display_n1} {op_latex['-']} {display_n2}"
    question_output = clean_latex_output(question_str)
    
    # 回傳結果
    return {'question_text': question_output, 'answer': str(result), 'mode': 1}
