...
# 中文名稱：二次函數圖形特性
# 英文名稱：Quadratic Function Properties
# 內容描述：根據 y=ax^2+bx+c 的 a 值判斷開口方向，並識別對稱軸 (x=−b/2a) 與頂點坐標。
...
def generate():
    ...
    if question_type == 'opening_direction':
        ...
        question_text = f"請問二次函數 {func_str} 的對稱軸方程式為何？ (請以 x=h 的格式回答)"
        # 分數表示
        ...
        question_text = f"請問二次函數 {func_str} 的頂點坐標為何？ (請以 (h,k) 的格式回答)"
        h = fractions.Fraction(-b, 2 * a)
        # 計算 k = a*h^2 + b*h + c
        k = a * (h**2) + b * h + c
