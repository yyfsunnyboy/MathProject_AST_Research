...
def poly_to_string(p_coeffs):
    ...
    for i, c in enumerate(p.coeffs):
        ...
        else: var_str = f"{coeff_str}x²" # 限制在二次
...
def generate(level=1):
    """生成多項式加減法題目 (圖形題)"""
    f_coeffs = [random.randint(-5, 5) for _ in range(random.randint(2, 3))]
    g_coeffs = [random.randint(-5, 5) for _ in range(random.randint(2, 3))]
    op = random.choice(['+', '-'])
    ...
    return {
        ...
        "context_string": f"計算多項式 {f_str} {op} {g_str}"
    }
