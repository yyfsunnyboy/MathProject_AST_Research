...
# Performance: 23.80s | Tokens: In=1653, Out=952
# Created At: 2026-01-28 13:02:33
...
This is a detailed specification for generating arithmetic addition problems involving two integers. The domain focuses on integer addition with constraints to ensure the problem's complexity and relevance. Here’s a breakdown of the key components:


    - **n1**: An integer with a value range from -100 to 100, excluding zero.
- **n2**: Another integer with the same constraints as n1.

- The only operator used is addition (`+`).

- **可計算性 (Calculability)**: All intermediate values and the final answer must be precisely calculable using integer operations.
- **邊界 (Boundaries)**:
  - Integer range for inputs `n1, n2`: -100 to 100.
  - Result range for `n1 + n2`: -200 to 200.
- **最小複雜度 (Minimum Complexity)**:
  - Must have two operands (`n1` and `n2`).
  - Both operands must be non-zero integers.
...
1. Randomly generate two positive integers, `abs_n1` and `abs_n2`, within the range 1 to 100.
...
4. Check if `final_answer` is within the range -200 to 200. If not, regenerate `abs_n1` and `abs_n2` and repeat steps 2-4.

- Ensure two non-zero integers (`n1`, `n2`) are generated.
- Confirm that `sign_combination` is randomly selected each time and covers all defined combinations.
- Verify the correct calculation of `n1 + n2`.
- Check if `final_answer` is within the specified range.
- Ensure all conditions for minimum complexity (non-zero integers) are met.
...
- **Answer Display**:
  - Convert `final_answer` to a string directly.
  - Avoid LaTeX formatting or using `fmt_num()`.

- The template focuses on generating addition problems with two non-zero integers, considering diverse sign combinations for variety.
...
- **Degradation Checks**:
  - Avoid zero as an operand by enforcing non-zero constraints in `entities` and minimum complexity.
  - Ensure the result is within a reasonable range to prevent overflow or excessively large values.

- After generation, verify:
  - `n1` and `n2` are non-zero integers within the range [-100, 100].
  - `final_answer` is an integer within the range [-200, 200].
...
This specification ensures a robust framework for generating diverse and valid integer addition problems suitable for educational purposes.