...
# Performance: 19.23s | Tokens: In=6015, Out=664
# Created At: 2026-01-28 10:26:19
# Fix Status: [Repaired] | Fixes: Regex=17, AST=0
...
def generate(level=1, **kwargs):
    ...
    if mode == 1:
        ...
        scene = safe_choice(['altitude', 'bank', 'temperature', 'direction', 'profit_loss'])
        action = safe_choice(['positive', 'negative'])
        ans = n1 if action == 'positive' else -n1
        q = apply_scenario(scene, action, n1=n1)
    
    elif mode == 2:
        n1 = random.randint(-1000, 1000)        action = 'positive' if n1 >= 0 else 'negative'
        scene = safe_choice(['interpret_magnitude_altitude', 'interpret_magnitude_bank', 'interpret_magnitude_direction'])
        ans = abs(n1)
        q = apply_scenario(scene, action, n1=abs(n1))
        ...
        action = safe_choice(['positive', 'negative'])
        ans = n1 if action == 'positive' else -n1
        event_word = "進步了" if action == 'positive' else "退步了"
        q = f"小明第一次考試 {n2} 分, 第二次考試{event_word} {n1} 分。若以成績進步為正, 應如何記錄這次成績變化？"
    
    elif mode == 4:
        n2 = random.randint(100, 1000)
        n1 = random.randint(max(1, n2 - 500), n2 + 500)
        scene_type = safe_choice(['calc_profit_loss', 'calc_score_change'])
        ans = n1 - n2
        action = 'positive' if ans >= 0 else 'negative'
        q = apply_scenario(scene_type, action, n1=n1, n2=n2)
    
    elif mode == 5:
        n1 = random.randint(-1000, 1000)        ans = abs(n1)
        q = f"計算: |{fmt_num(n1)}|"
    
    elif mode == 6:
        n1 = random.randint(-1000, 1000)        ans = -n1
        q = f"數字 {fmt_num(n1)} 的相反數是？"
