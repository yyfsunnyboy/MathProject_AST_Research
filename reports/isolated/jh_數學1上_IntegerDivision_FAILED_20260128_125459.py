...
# Performance: 38.05s | Tokens: In=1519, Out=1576
# Created At: 2026-01-28 12:54:59
...
def generate(level=1, **kwargs):
    ...
    json
    {
      ...
      "constraints": [
        ...
        {
          "boundary": [
            "* 整數範圍 (生成時): dividend 絕對值不超過 100, divisor 絕對值不超過 12。",
            "* 最終答案範圍: 最終的商 (quotient) 絕對值不超過 10。"
          ]
        },
        {
          "mutual_exclusion": [
        ...
        },
        {
          "minimum_complexity": [
            "* 必須包含兩個整數運算數。",
            "* 必须至少有一個運算數為負數 (dividend 或 divisor)。",
            "* 必须是整數除法。"
          ]
        }
      ],
      "templates": [
        {
          ...
          "complexity_requirements": [
            "- 必須包含兩個整數 (dividend, divisor)。",
            "- 必须執行一次除法運算。",
            "- 必须至少有一個運算數 (dividend 或 divisor) 為負數。",
            "- dividend 必须能被 divisor 整除。",
            "- 最終答案的絕對值必須在 1 到 10 之間。"
          ],
          "variables": [
            {"abs_divisor_val": "從 [2~12] 的正整數中隨機選取。"},
            {"abs_quotient_val": "從 [2~10] 的正整數中隨機選取。"},
            {"dividend_sign": "從 [-1, 1] 中隨機選取。"},
            {"divisor_sign": "從 [-1, 1] 中隨機選取。"}
          ],
          "construction": [
            "1. 隨機生成 abs_divisor_val (2~12) 和 abs_quotient_val (2~10)。",
            "2. 計算 abs_dividend_val = abs_divisor_val * abs_quotient_val。",
            "3. 隨機生成 dividend_sign 和 divisor_sign。",
            "4. 確保至少有一個 sign 是 -1 (即 dividend_sign 和 divisor_sign 不可同時為 1)。如果兩個都是 1, 則重新生成其中一個。",
            "5. 計算 dividend = dividend_sign * abs_dividend_val。",
            "6. 計算 divisor = divisor_sign * abs_divisor_val。",
            "7. 最終答案: result = dividend / divisor (使用 Python 的整數除法 `//` 或 `/` 並確保結果為整數)。"
          ],
          "implementation_checklist": [
            "- [ ] 是否生成了正確範圍的 abs_divisor_val 和 abs_quotient_val。",
            "- [ ] 是否正確計算了 abs_dividend_val。",
            "- [ ] 是否確保了 dividend_sign 和 divisor_sign 中至少有一個為 -1。",
            "- [ ] 是否正確計算了 dividend 和 divisor。",
            "- [ ] 是否確保 divisor 不為 0。",
            "- [ ] 是否確保 dividend 能夠被 divisor 整除。",
            "- [ ] 最終答案的絕對值是否在 1 到 10 之間。"
          ],
          "formatting": {
            "question_display": [
              "題幹格式化規則: ",
              "1. 使用 fmt_num() 格式化 dividend 和 divisor。fmt_num() 會自動為負數添加括號。",
              "2. 使用 op_latex['/'] 映射除號。",
              "3. 將數學式子部分用 clean_latex_output() 包裝。",
              "4. 將中文與數學式子拼接。",
              "",
              "範例: ",
              "question_math = f\"({fmt_num(dividend)}) {op_latex['/']} ({fmt_num(divisor)})"
            ],
            "answer_display": [
              "答案格式化規則: ",
              "- 最終答案為整數, 直接轉換為字符串。",
      ...
      "diversity": [
        ...
        {
          "value_size_variation": "透過隨機調整 abs_divisor_val 和 abs_quotient_val 的範圍來控制。"
        },
        {
          "degeneration_check": [
            "* 確保 divisor 不為 0。",
            "* 確保 dividend 必須能被 divisor 整除, 避免產生分數或餘數。",
            "* 確保至少有一個運算數為負數, 防止退化為純正數除法。",
            "* 確保最終答案的絕對值不為 1 (例如 5 ÷ 5), 提高題目複雜度。"
          ]
        }
      ],
      "verifier": [
        "- 生成後應驗證: ",
        "* `divisor` 不等於 0。",
        "* `dividend % divisor == 0` (確保整除)。",
        "* `result` 為整數。",
        "* `abs(result)` 介於 1 到 10 之間。",
        "* `(dividend < 0 or divisor < 0)` 為真 (確保至少有一個負數)。"
      ]
    }
