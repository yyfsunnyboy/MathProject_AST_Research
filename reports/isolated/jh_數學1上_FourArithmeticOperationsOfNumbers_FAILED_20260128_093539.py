...
# Performance: 50.57s | Tokens: In=7454, Out=1641
# Created At: 2026-01-28 09:35:39
# Fix Status: [Repaired] | Fixes: Regex=30, AST=0
...
def generate(level=1, **kwargs):
    ...
    num_operands = safe_choice([3, 4])
    operand_values = []
    
    for _ in range(num_operands):
        choice = random.choices(['integer', 'fraction', 'decimal'], weights=[0.3, 0.4, 0.3])[0]
        
        if choice == 'integer':
            value = Fraction(random.randint(-50, -1) if random.random() < 0.5 else random.randint(1, 50))
        elif choice == 'fraction':
            numerator = random.randint(-49, 49)
            denominator = random.randint(2, 20)
            while abs(numerator) >= denominator:
                numerator = random.randint(-49, 49)
            value = Fraction(numerator, denominator)
        else:  # decimal
            value = round(random.uniform(-10, -1) if random.random() < 0.5 else random.uniform(1, 10), safe_choice([1, 2]))
    
    operators = []
    for _ in range(num_operands - 1):
        op = safe_choice(['+', '-', '*', '/'])
        while op == '/' and operand_values[-1].denominator == 0:
            op = safe_choice(['+', '-', '*'])
        operators.append(op)
    
    parentheses_type = 'none' if num_operands == 3 else safe_choice(['left_group', 'right_group', 'inner_group'])

    # Step 2: Execute calculations based on MASTER_SPEC construction
    result = None
    
    if parentheses_type == 'left_group':
        temp = operand_values[0] + operand_values[1]
        for i in range(2, num_operands):
            if operators[i-2] == '+':
                temp += operand_values[i]
            elif operators[i-2] == '-':
                temp -= operand_values[i]
            elif operators[i-2] == '*':
                temp *= operand_values[i]
        ...
        temp = operand_values[-1]
        for i in range(num_operands - 2, -1, -1):
            if operators[i] == '+':
                temp += operand_values[i]
            elif operators[i] == '-':
                temp -= operand_values[i]
                ...
                temp /= operand_values[i]
        result = operand_values[0] + temp
    elif parentheses_type == 'inner_group' and num_operands == 4:
        temp1 = operand_values[1] * operand_values[2]
        if operators[0] == '+':
            temp2 = operand_values[0] + temp1
        else:
            temp2 = operand_values[0] - temp1
        result = temp2 * operand_values[3]
    else:  # none or other complex structure
        temp = operand_values[0]
    ...
    # Step 3: Format the question and answer based on MASTER_SPEC formatting
    q_str = ""
    
    if parentheses_type == 'left_group':
        q_str += f"({fmt_num(operand_values[0])} {op_latex[operators[0]]} {fmt_num(operand_values[1])})"
        for i in range(2, num_operands):
            q_str += f" {op_latex[operators[i-1]]} {fmt_num(operand_values[i])}"
    elif parentheses_type == 'right_group':
        q_str += f"{fmt_num(operand_values[0])} {op_latex[operators[0]]} ({fmt_num(operand_values[1])} {op_latex[operators[1]]} {fmt_num(operand_values[2])})"
        if num_operands == 4:
            q_str += f" {op_latex[operators[2]]} {fmt_num(operand_values[3])}"
    elif parentheses_type == 'inner_group' and num_operands == 4:
        q_str += f"{fmt_num(operand_values[0])} {op_latex[operators[0]]} ({fmt_num(operand_values[1])} {op_latex[operators[1]]} {fmt_num(operand_values[2])}) {op_latex[operators[2]]} {fmt_num(operand_values[3])}"
    else:
        q_str += f"{fmt_num(operand_values[0])} {op_latex[operators[0]]} {fmt_num(operand_values[1])}"
        for i in range(2, num_operands):
            q_str += f" {op_latex[operators[i-1]]} {fmt_num(operand_values[i])}"
    ...
    if result.denominator == 1:
        answer_text = str(result.numerator)
    else:
        answer_text = f"{result.numerator}/{result.denominator}"
    
    return {'question_text': clean_latex_output(q), 'correct_answer': answer_text, 'answer': answer_text, 'mode': 1}

def format_mixed_number_for_answer(fraction_obj):
    whole_part = fraction_obj.numerator // fraction_obj.denominator
    fractional_part = Fraction(abs(fraction_obj.numerator) % fraction_obj.denominator, fraction_obj.denominator)
