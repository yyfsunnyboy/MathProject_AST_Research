...
# Performance: 22.82s | Tokens: In=1653, Out=947
# Created At: 2026-01-28 12:51:18
...
def generate(level=1, **kwargs):
    ...
    This is a detailed specification for generating arithmetic addition problems involving two integers. The domain focuses on integer addition with constraints to ensure the problem's complexity and relevance. Here’s a breakdown of the key components:
        ...
        - **n1**: An integer with constraints that it must be within the range -100 to 100 and non-zero.
    ...
    - **最小複雜度 (Minimum Complexity)**:
      ...
      **Complexity Requirements**:
      ...
      - Ensure a diverse combination of signs for the operands (positive-positive, negative-negative, positive-negative, negative-positive).
      ...
      **Construction Steps**:
      ...
      2. Convert `abs_n1` and `abs_n2` into `n1` and `n2` based on the value of `sign_combination`.
         - 'pos_pos': `n1 = abs_n1`, `n2 = abs_n2`
         - 'neg_neg': `n1 = -abs_n1`, `n2 = -abs_n2`
         - 'pos_neg': `n1 = abs_n1`, `n2 = -abs_n2`
         - 'neg_pos': `n1 = -abs_n1`, `n2 = abs_n2`
      ...
      **Implementation Checklist**:
      ...
      - Confirm that `sign_combination` is randomly selected and covers all defined combinations.
      ...
      **Formatting**:
          - **Question Display**: Use `fmt_num(n1)` and `fmt_num(n2)` to format numbers, ensuring negative numbers have parentheses. Use `op_latex['+']` for the addition operator. Concatenate these into a LaTeX string using `clean_latex_output()`.
      - **Answer Display**: Convert `final_answer` directly to a string without any LaTeX formatting.

      **Notes**:
          - The template focuses on generating two non-zero integers with diverse sign combinations.
      - `fmt_num` automatically handles negative number parentheses for proper LaTeX output.

        - Variability in operand absolute values (1 to 100).
    - Diverse sign combinations of operands (positive-positive, negative-negative, positive-negative, negative-positive).

        - After generation, verify:
            - `n1` and `n2` are non-zero integers within the range [-100 to 100].
      - `final_answer` is an integer within the range [-200 to 200].
      - The sign combination of `n1` and `n2` matches the intended `sign_combination`.
      - The formatted question string adheres to LaTeX norms, with negative numbers correctly bracketed.

    This specification ensures that generated arithmetic addition problems are both diverse and meet educational standards.
