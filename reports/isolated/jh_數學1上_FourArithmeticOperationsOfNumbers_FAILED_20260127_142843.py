...
# Performance: 247.47s | Tokens: In=3472, Out=1054
# Created At: 2026-01-27 14:28:43
# Fix Status: [Repaired] | Fixes: Regex=5, AST=4
...
def generate(level=1, **kwargs):
    ...
    operand_types = [random.choice(['num_integer', 'num_rational', 'num_decimal']) for _ in range(num_operands)]
    operators_list = [random.choice(['+', '-', '*', '/']) for _ in range(num_operands - 1)]
    if '*' not in operators_list and '/' not in operators_list:
        operators_list[random.randint(0, num_operands - 2)] = random.choice(['*', '/'])
    operands = []
    for t in operand_types:
        if t == 'num_integer':
            operands.append(Fraction(random.randint(-20, 20), 1))
        elif t == 'num_rational':
            ...
            valid_decimals = [i / 10 ** j for i in range(-100, 101) for j in range(1, 4) if (10 ** j).bit_length() <= 3]
            operands.append(Fraction(random.choice(valid_decimals)))
    while any((op == '/' and operands[i + 1] == 0 for i, op in enumerate(operators_list))):
        idx = random.randint(0, num_operands - 2)
        if operators_list[idx] == '/':
            operands[idx + 1] = Fraction(random.randint(-20, 20), 1) if operand_types[idx + 1] == 'num_integer' else Fraction(random.randint(-50, 50), random.randint(2, 20))
    has_parentheses = random.choice([True, False])
    if has_parentheses:
        paren_pos = random.choice(['first_two', 'last_two', 'middle_two'])
        if paren_pos == 'first_two':
            operands[0] = (operands[0], operands[1])
            operators_list.pop(0)
        elif paren_pos == 'last_two':
            operands[-2] = (operands[-2], operands[-1])
            operators_list.pop(-1)
        elif paren_pos == 'middle_two':
            mid_idx = num_operands // 2
            operands[mid_idx - 1] = (operands[mid_idx - 1], operands[mid_idx])
            operators_list.pop(mid_idx - 1)
    result = operands[0]
    for i in range(1, len(operators_list)):
        if isinstance(result, tuple):
            result = result[0] * result[1] if operators_list[i - 1] == '*' else result[0] / result[1]
        if isinstance(operands[i], tuple):
            operands[i] = operands[i][0] * operands[i][1] if operators_list[i - 1] == '*' else operands[i][0] / operands[i][1]
        result = result + operands[i] if operators_list[i - 1] == '+' else result - operands[i]
    q_parts = []
    for i in range(len(operators_list)):
        q_parts.append(fmt_num(operands[i], operand_types[i]))
        q_parts.append(fmt_num(operators_list[i]))
    q_parts.append(fmt_num(operands[-1], operand_types[-1]))
    if has_parentheses:
        paren_pos = random.choice(['first_two', 'last_two', 'middle_two'])
        if paren_pos == 'first_two':
            q_parts[0] = f'({q_parts[0]} {fmt_num(operators_list[0])} {q_parts[1]})'
            del q_parts[1]
        elif paren_pos == 'last_two':
            q_parts[-2] = f'({q_parts[-2]} {fmt_num(operators_list[-1])} {q_parts[-1]})'
            del q_parts[-1]
        elif paren_pos == 'middle_two':
            mid_idx = len(q_parts) // 2
            q_parts[mid_idx - 1] = f'({q_parts[mid_idx - 1]} {fmt_num(operators_list[mid_idx - 1])} {q_parts[mid_idx]})'
            del q_parts[mid_idx]
    question = ' '.join(q_parts)
    answer = fmt_num(result, 'num_rational')
