...
# Performance: 10.70s | Tokens: In=1653, Out=402
# Created At: 2026-01-28 12:56:42
...
def generate(level=1, **kwargs):
    This is a structured domain definition for generating arithmetic addition problems involving two integers. The domain ensures that the generated questions are both educational and varied, covering different scenarios of integer addition with positive and negative numbers.

    
    1. **Entities**:
       - `n1` and `n2`: These are defined as integers with constraints to ensure they are within the range of -100 to 100 and non-zero.
   
    2. **Operators**:
       - The only operator used is addition (`+`).

    3. **Constraints**:
       - Ensures that all calculations can be performed exactly (integer operations).
       - Sets boundaries for input values (-100 to 100) and the result of the operation (-200 to 200).
       - Requires at least two operands, both non-zero integers.

    4. **Templates**:
       - `two_integer_addition`: This template specifies how to generate questions.
         - It requires generating two non-zero integers (`n1` and `n2`) with a specified sign combination (positive-positive, negative-negative, positive-negative, or negative-positive).
         - The final answer must be within the range of -200 to 200.

    5. **Implementation Checklist**:
       - Ensures that all conditions are met for generating valid questions.
   
    6. **Formatting**:
       - Specifies how questions and answers should be formatted using LaTeX, ensuring proper display of negative numbers with parentheses.

    7. **Diversity**:
       - Focuses on varying the absolute values of integers (1 to 100) and their sign combinations to maintain diversity in generated problems.
   
    8. **Verifier**:
       - Provides a checklist for verifying that all constraints are met after generating questions, ensuring correctness and adherence to specifications.

    This domain definition is designed to be comprehensive, covering various aspects of integer addition problem generation while maintaining educational value and preventing trivial cases like adding zero.
