...
# Performance: 19.25s | Tokens: In=6866, Out=691
# Created At: 2026-01-27 23:39:49
...
def generate(level=1, **kwargs):
    ...
    operand_fraction = Fraction(random.randint(-20, 20), random.randint(2, 20)).limit_denominator()
    while operand_fraction.denominator > 20 or operand_fraction.numerator == 0:
        operand_fraction = Fraction(random.randint(-20, 20), random.randint(2, 20)).limit_denominator()

    # Ensure it's not an integer
    if operand_fraction.denominator == 1:
        operand_fraction = Fraction(operand_fraction.numerator + 1, operand_fraction.denominator)

    operand_decimal = round(random.uniform(-20.0, -0.1), random.randint(1, 2)) if random.random() < 0.5 else round(random.uniform(0.1, 20.0), random.randint(1, 2))
    
    # Ensure non-zero
    # Ensure at least one negative number
    if operand_int > 0 and operand_fraction.numerator > 0 and operand_decimal > 0:
        operand_int = -operand_int

    operators = ['+', '-', '*', '/']
    operator1 = safe_choice(operators)
    while operator1 == '/' or (operator1 in ('*', '/') and operand2 == 0):
        operator1 = safe_choice(operators)

    # Step 2: Execute calculations based on MASTER_SPEC construction
    if operator1 == '+':
        result = operand_int + operand_fraction + operand_decimal
    elif operator1 == '-':
        result = operand_int - operand_fraction - operand_decimal
    elif operator1 == '*':
        result = (operand_int * operand_fraction) * operand_decimal
        ...
        while operand_decimal == 0 or operand_fraction.denominator == 0:
            operand_fraction = Fraction(random.randint(-20, 20), random.randint(2, 20)).limit_denominator()
        result = (operand_int / operand_fraction) / operand_decimal

    # Step 3: Build the question LaTeX using fmt_num and op_latex
    q = f"({fmt_num(operand_int)} {op_latex[operator1]} {fmt_num(operand_fraction)}) {op_latex['*']} {fmt_num(operand_decimal)}"
    ...
    if result.denominator == 1:
        ...
        a = f"{result.numerator}/{result.denominator}"
