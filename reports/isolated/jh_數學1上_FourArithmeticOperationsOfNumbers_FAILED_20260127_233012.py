...
# Performance: 21.00s | Tokens: In=6877, Out=750
# Created At: 2026-01-27 23:30:12
# Fix Status: [Repaired] | Fixes: Regex=19, AST=1
...
def generate(level=1, **kwargs):
    ...
    for _ in range(num_operands):
        for _safety_loop_var in range(1000):
            ...
            if denominator != 0 and (value % denominator == 0 or random.random() < 0.5):
                operand = Fraction(value, denominator)
                break
    ...
    for i in range(num_operands - 1):
        if bracket_type == 'left_group':
            if i == 0:
                q_parts.append(f'({fmt_num(operands[i])} {op_latex[operators[i]]} {fmt_num(operands[i + 1])})')
                result = operands[i] + operands[i + 1] if operators[i] == '+' else operands[i] - operands[i + 1]
            elif i == 1:
                q_parts.append(f'({q_parts[-1]} {op_latex[operators[i]]} {fmt_num(operands[i + 1])})')
                result = result + operands[i + 1] if operators[i] == '+' else result - operands[i + 1]
        elif bracket_type == 'right_group':
            if i == num_operands - 2:
                q_parts.append(f'{fmt_num(operands[i])} {op_latex[operators[i]]} ({fmt_num(operands[i + 1])})')
                result = operands[i] + operands[i + 1] if operators[i] == '+' else operands[i] - operands[i + 1]
            elif i < num_operands - 2:
                q_parts.append(f'{q_parts[-1]} {op_latex[operators[i]]} ({fmt_num(operands[i + 1])})')
                result = result + operands[i + 1] if operators[i] == '+' else result - operands[i + 1]
        else:
            q_parts.append(f'{fmt_num(operands[i])} {op_latex[operators[i]]} {fmt_num(operands[i + 1])}')
