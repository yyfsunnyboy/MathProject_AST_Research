...
# Performance: 22.19s | Tokens: In=1653, Out=886
# Created At: 2026-01-28 12:50:55
...
This is a structured domain definition for generating integer addition problems, specifically tailored to ensure the questions are both educational and engaging. Here's a breakdown of its components:
...
- The only operator used is `+` for addition.

- **可計算性 (Calculability)**: All intermediate values and the final answer must be precisely calculable using integer operations.
- **邊界 (Boundaries)**:
  - Input integers (`n1`, `n2`) should be within -100 to 100.
  - The result of `n1 + n2` should be within -200 to 200.
- **最小複雜度 (Minimum Complexity)**:
  - There must be two operands.
  - Both operands (`n1`, `n2`) must be non-zero integers.

- **two_integer_addition**:
  - This template ensures the generation of two non-zero integer operands and uses the addition operator. It also considers various combinations of signs for these operands to increase diversity in problem types.
  
  - **Complexity Requirements**: 
    - Must generate two non-zero integers (`n1`, `n2`).
    - Use the addition operator (`+`).
    - Ensure a variety of sign combinations (positive-positive, negative-negative, positive-negative, negative-positive) for the operands.
  ...
  - **Construction Process**:
    1. Randomly generate two positive integers (`abs_n1`, `abs_n2`) within the range of 1 to 100.
    2. Convert these absolute values into `n1` and `n2` based on the `sign_combination`.
    3. Calculate the final answer as `final_answer = n1 + n2`.
    4. Check if `final_answer` is within the range of -200 to 200. If not, regenerate `abs_n1` and `abs_n2`.

  - **Implementation Checklist**:
    - Ensure two non-zero integers are generated.
    ...
    - Ensure all conditions for minimum complexity are met.

  - **Formatting**:
    - **Question Display**: Use `fmt_num(n1)` and `fmt_num(n2)` to format numbers, ensuring negative numbers have parentheses. Use `op_latex['+']` for the addition operator and wrap the entire expression with `clean_latex_output()`.
    - **Answer Display**: Convert `final_answer` directly to a string without LaTeX formatting.

  - **Notes**:
    This template focuses on generating integer addition problems with non-zero integers, considering various sign combinations for diversity. The `fmt_num` tool automatically handles negative number bracketing for proper LaTeX output.

- Variations in the absolute values of operands (1 to 100).
- Different sign combinations (positive-positive, negative-negative, positive-negative, negative-positive) randomly selected.
- Degradation checks ensure no zero as an operand and that results are within a reasonable range.
...
- The formatted question string adheres to LaTeX standards, with negative numbers correctly bracketed.

This structured approach ensures that generated problems are both educational and varied, suitable for use in educational settings.