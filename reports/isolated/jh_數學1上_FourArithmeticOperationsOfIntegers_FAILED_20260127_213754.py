...
# Performance: 20.05s | Tokens: In=5746, Out=544
# Created At: 2026-01-27 21:37:54
# Fix Status: [Repaired] | Fixes: Regex=10, AST=0
...
def generate(level=1, **kwargs):
    ...
    if all(op > 0 for op in operands):
        index_to_negate = random.randint(0, len(operands) - 1)
        operands[index_to_negate] *= -1
    
    for _ in range(num_operators):
        operator = safe_choice(['+', '-', '*', '/'])
        operators.append(operator)
    
    # Ensure at least one multiplication or division
    while not any(op in ['*', '/'] for op in operators):
        index_to_change = random.randint(0, len(operators) - 1)
        operators[index_to_change] = safe_choice(['*', '/'])
    
    # Step 2: Execute calculations according to MASTER_SPEC
    current_value = operands[0]
    expression_parts = [fmt_num(current_value)]
    ...
    for i in range(num_operators):
        ...
        if operator == '/':
            while current_value % next_operand != 0:
                next_operand = random.randint(-100, 100)
                while next_operand == 0:
                    next_operand = random.randint(-100, 100)
        
        expression_parts.append(f" {op_latex[operator]} ")
        expression_parts.append(fmt_num(next_operand))
    ...
    q = ''.join(expression_parts)
    q = clean_latex_output(q)  # Only call once
