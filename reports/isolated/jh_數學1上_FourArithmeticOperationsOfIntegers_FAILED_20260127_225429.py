...
# Performance: 28.49s | Tokens: In=6296, Out=832
# Created At: 2026-01-27 22:54:29
...
def generate(level=1, **kwargs):
    ...
    while True:
        # [Auto-Healer] 反向推導目標值
    final_result = random.randint(-50, 50)
    if final_result == 0: final_result = 1  # 確保非零
        ...
        while True:
            op3 = safe_choice(['*', '/'])
            if (op3 == '*' and n4 != 0) or (op3 == '/' and n4 not in [0, 1, -1]):
                break
        
        # Step 2: Execute calculations according to MASTER_SPEC construction
        intermediate_val_after_op2 = None
        if op3 == '*':
            if final_result % n4 == 0:
                intermediate_val_after_op2 = final_result // n4
            else:
                continue
        elif op3 == '/':
            intermediate_val_after_op2 = final_result * n4
        
        if abs(intermediate_val_after_op2) > 100:
            continue
        
        intermediate_val_after_op1 = None
        if op2 == '+':
            intermediate_val_after_op1 = intermediate_val_after_op2 - n3
        elif op2 == '-':
            intermediate_val_after_op1 = intermediate_val_after_op2 + n3
        elif op2 == '*':
            if intermediate_val_after_op2 % n3 == 0:
                intermediate_val_after_op1 = intermediate_val_after_op2 // n3
            else:
                continue
        elif op2 == '/':
        ...
        if abs(intermediate_val_after_op1) > 100:
            continue
        
        if op1 == '+':
            n2 = intermediate_val_after_op1 - n1
        elif op1 == '-':
            n2 = n1 - intermediate_val_after_op1
        
        if abs(n2) > 20 or n2 == 0:
            continue
        
        # Step 3: Construct the question LaTeX using fmt_num and op_latex
        first_group_str = f"({fmt_num(n1)} {op_latex[op1]} {fmt_num(n2)})"
        math_expression = f"{first_group_str} {op_latex[op2]} {fmt_num(n3)} {op_latex[op3]} {fmt_num(n4)}"
        q = f"計算 {clean_latex_output(math_expression)} 的值。"
