...
# Performance: 143.97s | Tokens: In=3103, Out=591
# Created At: 2026-01-27 13:28:39
...
def generate(level=1, **kwargs):
    op_latex = {'+': '+', '-': '-', '*': '\\times', '/': '\\div'}
    operand_types = ['integer', 'rational', 'decimal', 'mixed_number']
    ...
    for _ in range(num_terms):
        ...
        if operand_type == 'integer':
            value = random.randint(-100, 100) if allow_negative_numbers else random.randint(1, 100)
        elif operand_type == 'rational':
            numerator = random.randint(-100, 100) if allow_negative_numbers else random.randint(1, 100)
            ...
            denominator = 10
            value = Fraction(numerator, denominator)
        elif operand_type == 'mixed_number':
            whole = random.randint(-100, 100) if allow_negative_numbers else random.randint(1, 100)
            numerator = random.randint(1, 9)
            denominator = random.randint(2, 20)
            value = Fraction(whole * denominator + numerator, denominator)
        operands.append(value)
    operator_symbols = ['+', '-', '*', '/']
    operators_sequence = [random.choice(operator_symbols) for _ in range(num_terms - 1)]
    parentheses_structure = random.choice(['none', 'left_group', 'right_group', 'inner_group'])
    if parentheses_structure == 'none':
        expression = operands[0]
        for i in range(len(operators_sequence)):
            op = operators_sequence[i]
            ...
            if op == '+':
                ...
                expression /= operand
    q = f'{fmt_num(operands[0])}'
    for i in range(len(operators_sequence)):
        op = operators_sequence[i]
        operand = operands[i + 1]
        q += f' {op_latex[op]} {fmt_num(operand)}'
    q = clean_latex_output(q)
    a = fmt_num(expression)
