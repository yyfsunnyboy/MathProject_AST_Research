...
# Performance: 22.29s | Tokens: In=1653, Out=891
# Created At: 2026-01-28 13:08:38
...
def generate(level=1, **kwargs):
    ...
    - **n2**: Another integer with similar constraints.
    ...
    - **邊界 (Boundaries)**:
      - Integer range for inputs `n1` and `n2`: -100 to 100.
      - Result range for `n1 + n2`: -200 to 200.
    - **最小複雜度 (Minimum Complexity)**:
      - Must have two operands (`n1`, `n2`).
      ...
      **Construction Process**:
      ...
      2. Convert `abs_n1` and `abs_n2` into `n1` and `n2` based on the value of `sign_combination`.
         ...
         - 'pos_neg': One is positive, one is negative.
         - 'neg_pos': One is negative, one is positive.
      3. Calculate the final answer: `final_answer = n1 + n2`.
      4. Ensure that `final_answer` is within the range of -200 to 200. If not, regenerate `abs_n1` and `abs_n2`.

      **Implementation Checklist**:
          - Confirm two non-zero integers (`n1`, `n2`) are generated.
      - Verify `sign_combination` has randomness and covers all defined combinations.
      - Ensure correct calculation of `n1 + n2`.
      - Check that `final_answer` is within the specified range.
      - Ensure all conditions for minimum complexity are met.

      **Formatting**:
          - **Question Display**: Use `fmt_num(n1)` and `fmt_num(n2)` to format numbers, adding brackets around negative numbers. Use `op_latex['+']` for the addition operator. Concatenate these into a LaTeX string using `clean_latex_output()`.
      - **Answer Display**: Convert `final_answer` directly to a string without any formatting.

      **Notes**:
          - The template focuses on generating two non-zero integers with diverse sign combinations.
      - `fmt_num` automatically handles negative number brackets for proper LaTeX output.

        - Variability in operand absolute values (1 to 100).
    - Random selection of four possible sign combinations.
    - Degradation checks ensure no zero operands and results within the specified range.

        - After generation, verify:
            - `n1` and `n2` are non-zero integers within [-100, 100].
      - `final_answer` is an integer within [-200, 200].
    ...
    This specification ensures a wide variety of arithmetic addition problems while maintaining simplicity and relevance.
