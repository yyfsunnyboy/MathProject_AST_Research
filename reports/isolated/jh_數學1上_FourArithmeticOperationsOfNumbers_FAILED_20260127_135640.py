...
# Performance: 189.44s | Tokens: In=3842, Out=817
# Created At: 2026-01-27 13:56:40
# Fix Status: [Repaired] | Fixes: Regex=23, AST=0
...
def generate(level=1, **kwargs):
    ...
    # 生成規則 (operand_rational 或 operand_integer, 可為正負數)
    n1 = Fraction(random.randint(-1000, 1000), random.randint(1, 20))
    n2 = Fraction(random.randint(-1000, 1000), random.randint(1, 20))
    n3 = Fraction(random.randint(-1000, 1000), random.randint(1, 20))
    ...
    # 隨機選擇括號結構
    parentheses_structure = random.choice(['no', 'left', 'right', 'both'])

    # 計算最終結果
    if parentheses_structure == 'no':
        intermediate_result = safe_eval(f"{n1} {op1} {n2}") {op2} {n3}")
    elif parentheses_structure == 'left':
        intermediate_result = safe_eval(f"({n1} {op1} {n2}) {op2} {n3}")
    elif parentheses_structure == 'right':
        intermediate_result = safe_eval(f"{n1} {op1} ({n2} {op2} {n3})")
    elif parentheses_structure == 'both':
        intermediate_result = safe_eval(f"({n1} {op1} {n2}) {op2} ({n3})")

    # 確保最終答案已化為最簡形式
    final_result = intermediate_result.limit_denominator()

    # 格式化題幹和答案
    if parentheses_structure == 'no':
        question = f"${fmt_num(n1)} {op_latex[op1]} {fmt_num(n2)} {op_latex[op2]} {fmt_num(n3)}$"
    elif parentheses_structure == 'left':
        question = f"$({fmt_num(n1)} {op_latex[op1]} {fmt_num(n2)}) {op_latex[op2]} {fmt_num(n3)}$"
    elif parentheses_structure == 'right':
        question = f"${fmt_num(n1)} {op_latex[op1]} ({fmt_num(n2)} {op_latex[op2]} {fmt_num(n3)})$"
    ...
    answer = fmt_num(final_result, 'fraction')
