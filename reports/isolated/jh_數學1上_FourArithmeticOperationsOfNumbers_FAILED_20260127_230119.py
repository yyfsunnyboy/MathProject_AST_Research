...
# Performance: 30.84s | Tokens: In=7382, Out=1031
# Created At: 2026-01-27 23:01:19
# Fix Status: [Repaired] | Fixes: Regex=15, AST=0
...
def generate(level=1, **kwargs):
    ...
    operand_types = [safe_choice(['integer', 'fraction', 'decimal']) for _ in range(num_operands)]
    operators = ['+', '-', '*', '/']
    operator_choices = random.sample(operators, num_operands - 1)
    
    operands = []
    for i in range(num_operands):
        if operand_types[i] == 'integer':
            value = random.randint(-100, 100)
            while value == 0:
                value = random.randint(-100, 100)
            operands.append(Fraction(value))
        elif operand_types[i] == 'fraction':
        ...
        elif operand_types[i] == 'decimal':
            value = round(random.uniform(-100, 100), 2)
            while value == 0:
                value = round(random.uniform(-100, 100), 2)
            operands.append(Fraction(value).limit_denominator())
    
    # Ensure at least one negative number
    if all(op >= 0 for op in operands):
        operands[random.randint(0, num_operands - 1)] *= -1
    
    # Ensure at least one fraction or decimal
    if all(isinstance(op, int) for op in operands):
        ...
        while numerator == 0 or denominator == 0:
            numerator = random.randint(-100, 100)
            denominator = random.randint(2, 20)
        operands[index] = Fraction(numerator, denominator)
    
    # Ensure at least one multiplication or division operator
    if all(op not in ['*', '/'] for op in operator_choices):
        index = random.randint(0, len(operator_choices) - 1)
        operator_choices[index] = safe_choice(['*', '/'])
    
    has_parentheses = safe_choice([True, False])
    parenthesis_location = None
    if has_parentheses:
        parenthesis_location = safe_choice(['left_group', 'right_group', 'middle_group'])
    
    # Step 2: Execute calculations according to MASTER_SPEC construction
    final_result = operands[0]
    q_parts = [fmt_num(operands[0])]
    
    for i in range(num_operands - 1):
        op = operator_choices[i]
        next_operand = operands[i + 1]
        
        if op == '+':
            final_result += next_operand
        elif op == '-':
            final_result -= next_operand
        elif op == '*':
            final_result *= next_operand
            ...
            while next_operand == 0:
                next_operand = random.randint(-100, 100)
                if operand_types[i + 1] == 'fraction':
                    denominator = random.randint(2, 20)
                    while denominator == 0:
                        denominator = random.randint(2, 20)
                    next_operand = Fraction(next_operand, denominator)
            final_result /= next_operand
        
        q_parts.append(op_latex[op])
        q_parts.append(fmt_num(next_operand))
    
    # Step 3: Construct question LaTeX using fmt_num and op_latex
    if has_parentheses:
        if parenthesis_location == 'left_group':
            math_str = f"({q_parts[0]} {q_parts[1]} {q_parts[2]}) {' '.join(q_parts[3:])}"
        elif parenthesis_location == 'right_group':
            math_str = f"{' '.join(q_parts[:-2])} ({' '.join(q_parts[-2:])})"
        else:  # middle_group
            math_str = f"{' '.join(q_parts[:2])} ({' '.join(q_parts[2:])})"
    else:
        math_str = ' '.join(q_parts)
    
    question_display_output = "計算下列各式的值。" + clean_latex_output(math_str)
    
    # Step 4: Format the answer according to MASTER_SPEC formatting
    if final_result.denominator == 1:
        answer_display_output = str(final_result.numerator)
    else:
        answer_display_output = f"{final_result.numerator}/{final_result.denominator}"
