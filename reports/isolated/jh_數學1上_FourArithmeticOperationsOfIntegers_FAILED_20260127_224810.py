...
# Performance: 23.49s | Tokens: In=6296, Out=660
# Created At: 2026-01-27 22:48:10
# Fix Status: [Repaired] | Fixes: Regex=15, AST=1
...
def generate(level=1, **kwargs):
    ...
    for _safety_loop_var in range(1000):
        ...
        if n2 == 0:
            continue
        n3 = safe_choice([-9, -8, -7, -6, -5, -4, -3, -2, 2, 3, 4, 5, 6, 7, 8, 9])
        n4 = safe_choice([-9, -8, -7, -6, -5, -4, -3, -2, 2, 3, 4, 5, 6, 7, 8, 9])
        op1 = safe_choice(['+', '-'])
        op2 = safe_choice(['+', '-', '*', '/'])
        op3 = safe_choice(['*', '/'])
        if op3 == '*':
            intermediate_val_after_op2 = final_result // n4
            if final_result % n4 != 0 or abs(intermediate_val_after_op2) > 100:
                continue
        elif op3 == '/':
            intermediate_val_after_op2 = final_result * n4
        if op2 == '+':
            ...
            intermediate_val_after_op1 = intermediate_val_after_op2 // n3
            if intermediate_val_after_op2 % n3 != 0 or abs(intermediate_val_after_op1) > 100:
                continue
        elif op2 == '/':
            intermediate_val_after_op1 = intermediate_val_after_op2 * n3
        if op1 == '+':
        ...
        q = clean_latex_output(f'計算 {math_expression} 的值。')
        a = str(final_result)
