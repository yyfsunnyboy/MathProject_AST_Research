...
# Performance: 35.03s | Tokens: In=6866, Out=1191
# Created At: 2026-01-27 23:32:29
# Fix Status: [Repaired] | Fixes: Regex=13, AST=8
...
def generate(level=1, **kwargs):
    ...
    def ensure_fraction_or_decimal(operands):
        for operand in operands:
            if isinstance(operand, (Fraction, float)):
                return True
        return False

    def ensure_negative(operands):
        for operand in operands:
            if operand < 0:
                return True
        return False
    ...
    def gen_bracket_type():
        return safe_choice(['none', 'left_group', 'right_group'])

    def calculate_chain_of_operations(operands, operators, bracket_type):
        if bracket_type == 'left_group':
            temp_res = operands[0] + operands[1]
            final_result = safe_divide(temp_res * operands[2], 1)
        elif bracket_type == 'right_group':
            temp_res = operands[1] + operands[2]
            final_result = safe_divide(operands[0] * temp_res, 1)
        elif operators[0] in ['*', '/']:
            temp_res = safe_divide(operands[0] * operands[1], 1)
            final_result = safe_divide(temp_res + operands[2], 1)
        else:
            temp_res = safe_divide(operands[0] + operands[1], 1)
            final_result = safe_divide(temp_res * operands[2], 1)
    ...
    bracket_type = gen_bracket_type() if num_operands == 3 else 'none'
    operands = []
    for _ in range(num_operands):
        operand_type = safe_choice(['operand_int', 'operand_fraction', 'operand_decimal'])
    ...
    while not ensure_fraction_or_decimal(operands) or not ensure_negative(operands):
        for i in range(num_operands):
            if isinstance(operands[i], int):
                operands[i] = gen_operand_int()
            elif isinstance(operands[i], Fraction):
                operands[i] = gen_operand_fraction()
            else:
                operands[i] = gen_operand_decimal()
    operators = [gen_operator() for _ in range(num_operands - 1)]
    while not any((op in ['*', '/'] for op in operators)) or not ensure_negative(operators):
        for i in range(len(operators)):
            operators[i] = gen_operator()
    final_result = calculate_chain_of_operations(operands, operators, bracket_type)
    a = fmt_num(final_result)
    temp_parts = []
    temp_parts.append(fmt_num(operands[0]))
    for i in range(len(operators)):
        temp_parts.append(op_latex[operators[i]])
        temp_parts.append(fmt_num(operands[i + 1]))
    if bracket_type == 'left_group':
