...
# Performance: 252.69s | Tokens: In=3472, Out=1077
# Created At: 2026-01-27 14:37:55
...
def generate(level=1, **kwargs):
    ...
    operand_types = [safe_choice(['num_integer', 'num_rational', 'num_decimal']) for _ in range(num_operands)]
    operators_list = [safe_choice(['+', '-', '*', '/']) for _ in range(num_operands - 1)]
    operands = []
    for t in operand_types:
        if t == 'num_integer':
            ...
            while n == 0:
                n = random.randint(-20, 20)
            operands.append(Fraction(n))
        elif t == 'num_rational':
            num = random.randint(-50, 50)
            denom = safe_choice([i for i in range(2, 21) if gcd(num, i) != i])
            while denom == 0:
                denom = safe_choice([i for i in range(2, 21) if gcd(num, i) != i])
            operands.append(Fraction(num, denom))
        elif t == 'num_decimal':
            num = random.randint(-10, 10)
            denom = 10 ** random.randint(1, 3)
            while num * denom % denom != 0:
                num = random.randint(-10, 10)
            operands.append(Fraction(num, denom))
    has_parentheses = safe_choice([True, False])
    if has_parentheses:
        paren_pos = safe_choice(['first_two', 'last_two', 'middle_two'])
    else:
        paren_pos = None
    expression = []
    for i in range(len(operands) - 1):
        expression.append(fmt_num(operands[i]))
        expression.append(op_latex[operators_list[i]])
    expression.append(fmt_num(operands[-1]))
    if paren_pos == 'first_two':
        q = f'({expression[0]} {op_latex[operators_list[0]]} {expression[1]}) {op_latex[operators_list[1]]} {expression[2]}'
    elif paren_pos == 'last_two':
        q = f'{expression[0]} {op_latex[operators_list[0]]} ({expression[1]} {op_latex[operators_list[1]]} {expression[2]})'
    elif paren_pos == 'middle_two':
        q = f'{expression[0]} {op_latex[operators_list[0]]} ({expression[1]} {op_latex[operators_list[1]]} {expression[2]}) {op_latex[operators_list[2]]} {expression[3]}'
    else:
        q = ' '.join(expression)
    result = operands[0]
    for i in range(len(operators_list)):
        if operators_list[i] == '+':
            ...
            while operands[i + 1] == 0:
                num = random.randint(-50, 50)
                denom = safe_choice([i for i in range(2, 21) if gcd(num, i) != i])
                operands[i + 1] = Fraction(num, denom)
            result /= operands[i + 1]
    q = clean_expression(q)
    a = fmt_num(result)
    return {'question': q, 'answer': a}

def clean_expression(expr):
    expr = expr.replace(' + ', ' +').replace(' - ', ' -').replace(' * ', ' \\times ').replace(' / ', ' \\div ')
    return expr

def to_fraction(value, original_type):
    if isinstance(value, int):
        return Fraction(value)
    elif isinstance(value, float):
        num = value.as_integer_ratio()[0]
        denom = value.as_integer_ratio()[1]
        return Fraction(num, denom)
    elif isinstance(value, tuple) and len(value) == 2:
        whole, frac = value
        num = whole * frac[1] + frac[0]
        denom = frac[1]
        return Fraction(num, denom)

def simplify_fraction(fraction_obj):
    return fraction_obj.limit_denominator()