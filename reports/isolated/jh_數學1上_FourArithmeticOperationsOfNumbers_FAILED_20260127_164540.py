...
# Performance: 23.21s | Tokens: In=3649, Out=742
# Created At: 2026-01-27 16:45:40
# Fix Status: [Repaired] | Fixes: Regex=9, AST=0
...
def generate(level=1, **kwargs):
    ...
    operands_data = [
        {"type": "integer", "range": [-100, 100], "exclude": [0]},
        {"type": "fraction", "numerator_range": [-20, 20], "denominator_range": [2, 20], "exclude_integers": True},
        {"type": "decimal", "range": [-5, 5], "decimal_places": [1, 2]},
        {"type": "mixed_number", "integer_range": [-5, 5], "numerator_range": [1, 10], "denominator_range": [2, 10]}
    ]
    
    operators_sequence = random.choices(['+', '-', '*', '/'], k=num_terms - 1)
    parentheses_structure = safe_choice(['none', 'left_grouped', 'right_grouped'])
    if num_terms >= 4:
        parentheses_structure = safe_choice(['none', 'left_grouped', 'right_grouped', 'all_grouped'])

    operands = []
    for data in operands_data[:num_terms]:
        if data['type'] == "integer":
            operand = Fraction(random.randint(data['range'][0], data['range'][1]))
        elif data['type'] == "fraction":
            numerator = random.randint(data['numerator_range'][0], data['numerator_range'][1])
            denominator = safe_choice([d for d in range(2, 21) if gcd(numerator, d) != 1 or not data['exclude_integers']])
            operand = Fraction(numerator, denominator)
        elif data['type'] == "decimal":
            decimal_places = random.randint(data['decimal_places'][0], data['decimal_places'][1])
            operand = Fraction(round(random.uniform(data['range'][0], data['range'][1]), decimal_places))
        elif data['type'] == "mixed_number":
            integer_part = random.randint(data['integer_range'][0], data['integer_range'][1])
            numerator = random.randint(1, data['numerator_range'][1])
            denominator = safe_choice([d for d in range(2, 21) if gcd(numerator, d) != 1 or not data['exclude_integers']])
            operand = Fraction(integer_part * denominator + numerator, denominator)
        operands.append(operand)

    expression = []
    if parentheses_structure == 'left_grouped':
        expression.append('(')
    for i in range(num_terms):
        expression.append(fmt_num(operands[i]))
        if i < num_terms - 1:
            expression.append(op_latex(operators_sequence[i]))
    if parentheses_structure == 'left_grouped':
        expression.append(')')
    elif parentheses_structure == 'right_grouped':
        expression.insert(-2, '(')
    ...
    return {'question_text': f'${q}$', 'correct_answer': f, 'answer': f, 'mode': 1'${a}$'}