...
# Performance: 25.16s | Tokens: In=1736, Out=1004
# Created At: 2026-01-28 12:53:21
...
This domain and template are designed for generating arithmetic problems focusing on integer multiplication, particularly suitable for K12 mathematics education. Here's a breakdown of the key components:


    - **factor1**: An integer with constraints that its value must be between -10 and 10 (inclusive) and cannot be zero.
- **factor2**: Another integer with similar constraints as factor1.

- Multiplication (`*`).

- The problem must ensure all intermediate values and the final answer are integers.
- Integer ranges:
  - Factors: -10 to 10
  - Result: -100 to 100 (since the maximum product of two factors within this range is 10 * 10 = 100, and the minimum is -10 * 10 = -100).
- The absolute value of the result should not exceed three digits.
- No mutual exclusions are specified.
- Minimum complexity:
  - Must include a multiplication operation between two non-zero integers.
  - Must be able to generate all four combinations of positive and negative signs (positive x positive, positive x negative, negative x positive, negative x negative).



- Generate two non-zero integers as multipliers and multiplicands.
- Use the multiplication operation.
- Ensure that generated problems cover all four combinations of sign possibilities (e.g., random generation should ensure each combination has an equal probability).
- Each factor's absolute value must be at least 1.

- **factor1_abs**: An integer randomly selected from 1 to 10.
- **factor2_abs**: Another integer randomly selected from 1 to 10.
- **factor1_sign**: A boolean (True for positive, False for negative) randomly selected.
- **factor2_sign**: Similarly, a boolean randomly selected.

1. Randomly generate `factor1_abs` and `factor2_abs`, ensuring they are integers between 1 and 10.
2. Randomly generate `factor1_sign` and `factor2_sign`.
3. Determine the value of `factor1` based on `factor1_abs` and `factor1_sign`.
4. Similarly, determine the value of `factor2` based on `factor2_abs` and `factor2_sign`.
5. Calculate the final answer using Python's multiplication operator.
6. Ensure that the final answer is an integer within the expected range.

- Confirm two non-zero integers are generated as factors.
- Verify each factor's absolute value is between 1 and 10.
- Check if the sign of each factor is correctly set based on boolean values.
- Use Python’s multiplication operator to calculate the result.
- Ensure that over time, all four combinations of positive and negative signs are covered.
- Confirm the final answer is an integer with an absolute value not exceeding 100.
...
- **Question Display**:
  ...
  - Use LaTeX to represent the multiplication operator (`\\times`).
  - Combine formatted factors and operators into a string, then wrap it in `$ ... $`.
- **Answer Display**:
  - Convert the final answer directly to a string without using LaTeX or `fmt_num()`.

- This template focuses on basic integer multiplication concepts, especially handling positive and negative signs.
- The automatic addition of parentheses for negative numbers by `fmt_num` is crucial for maintaining clear problem presentation.

- **Variation 1**: Ensure all four combinations of factor sign types (positive x positive, positive x negative, negative x positive, negative x negative) are randomly generated.
- **Variation 2**: The absolute values of the two factors can independently vary between 1 and 10.
- **Degradation Check**:
  - Ensure that neither factor is zero to avoid simple multiplication by zero scenarios.
  - Avoid overly simple multiplications where both factors are either 1 or -1. At least one factor should have an absolute value greater than 1.

After generation, verify the following:
    - `factor1` and `factor2` are non-zero integers.
- The absolute values of `factor1` and `factor2` are between 1 and 10.
- The final answer is an integer with an absolute value not exceeding 100.
- The mathematical expression in the question has correct formatting, including proper parentheses for negative numbers.
- The answer is displayed as a plain numeric string.

This structured approach ensures that generated arithmetic problems meet educational standards while providing variety and complexity suitable for K12 learners.