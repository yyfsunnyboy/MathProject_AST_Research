...
# Performance: 17.92s | Tokens: In=3396, Out=545
# Created At: 2026-01-27 17:38:37
# Fix Status: [Repaired] | Fixes: Regex=11, AST=2
...
def generate(level=1, **kwargs):
    ...
    def generate_operands(target_result, operator, operand_constraints):
        for _safety_loop_var in range(1000):
            if operator in ['+', '-']:
                operand_right = random.randint(*operand_constraints['range'])
                if operand_right == 0:
                    continue
                operand_left = target_result - operand_right if operator == '+' else target_result + operand_right
            elif operator == '*':
                operand_right = safe_choice([x for x in range(*operand_constraints['multiplier_range']) if x not in [1, -1]])
                if target_result % operand_right != 0:
                    continue
                operand_left = target_result // operand_right
            elif operator == '/':
                operand_right = safe_choice([x for x in range(*operand_constraints['divisor_range']) if x not in [1, -1]])
                operand_left = target_result * operand_right
            if all((-100 <= x <= 100 for x in [operand_left, operand_right])):
                return (operand_left, operand_right)
        return (0, 0)
    final_ans = random.randint(-50, 50)
    while final_ans == 0:
        final_ans = random.randint(-50, 50)
    op3 = safe_choice(['+', '-', '*', '/'])
    intermediate_result1, n4 = generate_operands(final_ans, op3, {'range': [-30, 30], 'multiplier_range': [-5, 5], 'divisor_range': [-5, 5]})
    op2 = safe_choice(['+', '-', '*', '/'])
    inner_result, n3 = generate_operands(intermediate_result1, op2, {'range': [-30, 30], 'multiplier_range': [-5, 5], 'divisor_range': [-5, 5]})
    op1 = safe_choice(['+', '-', '*', '/'])
    n1, n2 = generate_operands(inner_result, op1, {'range': [-30, 30], 'multiplier_range': [-5, 5], 'divisor_range': [-5, 5]})
    q = f'$( {fmt_num(n1)} {op_latex[op1]} {fmt_num(n2)}) {op_latex[op2]} {fmt_num(n3)} {op_latex[op3]} {fmt_num(n4)}'
