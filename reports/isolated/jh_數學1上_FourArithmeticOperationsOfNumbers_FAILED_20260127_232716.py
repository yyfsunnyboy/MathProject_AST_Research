...
# Performance: 31.80s | Tokens: In=6877, Out=904
# Created At: 2026-01-27 23:27:16
...
def generate(level=1, **kwargs):
    ...
    for _ in range(num_operands):
            denominator = random.randint(1, 20) if level > 1 else 1
            rational_number = Fraction(value, denominator)
            if (rational_number.denominator != 1 or safe_choice([True, False])) and rational_number.numerator * rational_number.denominator <= 999:
                operands.append(rational_number)
                break
    
    operators = []
    for _ in range(num_operators):
        operator = safe_choice(['+', '-', '*', '/'])
        if operator == '/' and (operands[-1] == 0 or operands[-2] == 0):
            continue
        operators.append(operator)

    bracket_type = safe_choice(['none', 'left_group', 'right_group'])
    ...
    if bracket_type == 'left_group':
        ...
        temp_result = operands[-1]
        for i in range(num_operators - 1, -1, -1):
            if operators[i] == '+':
                temp_result += operands[i]
            elif operators[i] == '-':
                temp_result -= operands[i]
            elif operators[i] == '*':
                temp_result *= operands[i]
    ...
    if bracket_type == 'left_group':
        for i in range(num_operands - 1):
            q += f"({fmt_num(operands[i])} {op_latex[operators[i]]} {fmt_num(operands[i + 1])})"
            if i < num_operators - 2:
                q += " "
    elif bracket_type == 'right_group':
        for i in range(num_operands - 1, 0, -1):
            q += f"{fmt_num(operands[i])} {op_latex[operators[i - 1]]} ({fmt_num(operands[i - 1])})"
            if i > 1:
                q += " "
    else:
        for i in range(num_operands - 1):
    ...
    if result.numerator > result.denominator and result.denominator != 1:
        ...
        a = f"{whole} {rem}/{result.denominator}"
