...
# Performance: 448.69s | Tokens: In=5875, Out=1457
# Created At: 2026-01-26 23:23:45
# Fix Status: [Repaired] | Fixes: Regex=3, AST=2
...
def generate(level=1, **kwargs):
    template = random.choice(['chained_arithmetic_operations', 'distributive_property_application'])
    if template == 'chained_arithmetic_operations':
        num_operands = random.randint(3, 4)
        operand_types = ['integer', 'fraction', 'decimal', 'mixed_number']
        operand_values = []
        operators_selected = []
        for _ in range(num_operands):
            operand_type = random.choice(operand_types)
            if operand_type == 'integer':
                value = Fraction(random.randint(-50, 50))
                while value == 0:
                    value = Fraction(random.randint(-50, 50))
            elif operand_type == 'fraction':
                numerator, denominator = generate_coprime_pair(50, 20)
                value = Fraction(numerator, denominator)
            elif operand_type == 'decimal':
                value = generate_decimal_fraction(20, 2)
            else:
                whole_part, numerator, denominator = generate_mixed_number_parts(20, 50, 20)
                value = Fraction(whole_part * denominator + numerator, denominator)
            operand_values.append(value)
        for _ in range(num_operands - 1):
            operators_selected.append(random.choice(['+', '-', '*', '/']))
        has_parentheses = random.choice([True, False])
        if not has_parentheses:
            i = 0
            while i < len(operators_selected):
                op = operators_selected[i]
                if op in ('*', '/'):
                    if op == '*':
                        operand_values[i] *= operand_values[i + 1]
                    else:
                        operand_values[i] /= operand_values[i + 1]
                    del operand_values[i + 1]
                    del operators_selected[i]
                else:
                    i += 1
            for i in range(len(operators_selected)):
                op = operators_selected[i]
                if op == '+':
                    operand_values[i] += operand_values[i + 1]
                else:
            ...
            i = random.randint(0, len(operators_selected) - 1)
            op = operators_selected[i]
            if op == '*':
                operand_values[i] *= operand_values[i + 1]
            elif op == '/':
        ...
        q = fmt_num(operand_values, operators_selected, has_parentheses)
        q = clean_latex_output(q)
        a = fmt_num(result)
    else:
        factor_A_type = random.choice(['integer', 'fraction', 'mixed_number'])
        if factor_A_type == 'integer':
            factor_A = Fraction(random.randint(-50, 50))
            while factor_A == 0:
                factor_A = Fraction(random.randint(-50, 50))
        elif factor_A_type == 'fraction':
            numerator, denominator = generate_coprime_pair(50, 20)
            factor_A = Fraction(numerator, denominator)
        else:
            whole_part, numerator, denominator = generate_mixed_number_parts(20, 50, 20)
            factor_A = Fraction(whole_part * denominator + numerator, denominator)
        factor_B_type = random.choice(['integer', 'fraction', 'mixed_number'])
        if factor_B_type == 'integer':
            factor_B = Fraction(random.randint(-50, 50))
            while factor_B == 0:
                factor_B = Fraction(random.randint(-50, 50))
        elif factor_B_type == 'fraction':
            numerator, denominator = generate_coprime_pair(50, 20)
            factor_B = Fraction(numerator, denominator)
        else:
            whole_part, numerator, denominator = generate_mixed_number_parts(20, 50, 20)
            factor_B = Fraction(whole_part * denominator + numerator, denominator)
        common_factor_C_type = random.choice(['integer', 'fraction'])
        if common_factor_C_type == 'integer':
            common_factor_C = Fraction(random.randint(-1000, 1000))
            while common_factor_C in (1, -1):
                common_factor_C = Fraction(random.randint(-1000, 1000))
        else:
            numerator, denominator = generate_coprime_pair(50, 20)
            common_factor_C = Fraction(numerator, denominator)
        operation_sign = random.choice(['+', '-'])
        order_of_factors = random.choice(['C_first', 'C_last'])
        if operation_sign == '+':
            result = (factor_A + factor_B) * common_factor_C
        else:
            result = (factor_A - factor_B) * common_factor_C
        q = fmt_num(factor_A, factor_B, common_factor_C, operation_sign, order_of_factors)
        q = clean_latex_output(q)
        a = fmt_num(result)
    if isinstance(q, str):
