...
# Performance: 31.84s | Tokens: In=6866, Out=1151
# Created At: 2026-01-27 23:41:44
...
def generate(level=1, **kwargs):
    ...
    bracket_type = None if num_operands == 4 else safe_choice(['none', 'left_group', 'right_group'])
    
    operands = []
    for _ in range(num_operands):
        operand_type = Fraction(safe_choice(['operand_int', 'operand_fraction', 'operand_decimal']))
        if, operand_type = Fraction(= 'operand_int':)
            operands.append(gen_operand_int())
        elif, operand_type = Fraction(= 'operand_fraction':)
    ...
    # 確保至少一個是負數
    while all(op > 0 for op in operands) and all(op < 0 for op in operands):
        idx = random.randint(0, num_operands - 1)
        operands[idx] *= -1
    ...
    # 確保至少一個是乘法或除法
    while all(op not in ['*', '/'] for op in operators):
        idx = random.randint(0, num_operands - 2)
        if operators[idx] not in ['*', '/']:
            operators[idx] = gen_operator()

    # 處理除零情況
    for i in range(num_operands - 1):
        while (operators[i] == '/' and operands[i + 1] == 0) or (operands[i] == 0 and operators[i] == '/'):
            idx = random.randint(0, num_operands - 2)
            if operators[idx] == '/' and operands[idx + 1] == 0:
                operands[idx + 1] = gen_operand_int()
            elif operands[idx] == 0 and operators[idx] == '/':
                operands[idx] = gen_operand_int()

    # 計算最終答案
    final_result = operands[0]
    for i in range(1, num_operands):
        if operators[i - 1] == '+':
            final_result += operands[i]
        elif operators[i - 1] == '-':
            final_result -= operands[i]
        elif operators[i - 1] == '*':
            final_result *= operands[i]
    ...
    for i in range(num_operands):
        if isinstance(operands[i], Fraction) and operands[i].denominator > 1:
            temp_parts.append(fmt_num(Fraction(operands[i])))
        elif isinstance(operands[i], float):
            temp_parts.append(fmt_num(float(operands[i])))
        else:
            temp_parts.append(fmt_num(int(operands[i])))
    ...
    if isinstance(final_result, Fraction):
        a = str(final_result)
    elif final_result.is_integer():
        a = str(int(final_result))
    else:
        whole = int(final_result // 1)
        rem = Fraction(final_result % 1).limit_denominator()
        a = f"{whole} {rem.numerator}/{rem.denominator}"

    return {
        'question_text': clean_latex_output(q),
        'correct_answer': a,
        'answer': a,
        'mode': 1
    }