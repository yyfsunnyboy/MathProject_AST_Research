...
# Performance: 15.74s | Tokens: In=1653, Out=654
# Created At: 2026-01-28 11:32:49
...
def generate(level=1, **kwargs):
    This domain and template are designed to generate arithmetic addition problems involving two non-zero integers. The constraints ensure that the numbers used in the problem are within a specified range, and the operations result in an answer also within a defined range. Here's a breakdown of the key components:

    
        - **n1**: An integer with constraints to be between -100 and 100 (inclusive) and non-zero.
    - **n2**: Another integer with similar constraints.

        - The only operator used is the addition symbol (`+`).

        - All intermediate values and the final answer must be integers.
    - The input numbers `n1` and `n2` should be between -100 and 100, inclusive.
    - The result of `n1 + n2` should be between -200 and 200, inclusive.
    - Both operands (`n1` and `n2`) must be non-zero integers.

        - **Name**: `two_integer_addition`
  
      This template ensures that the generated problem meets specific criteria for complexity and diversity. It includes a variable `sign_combination` to control the sign combinations of `n1` and `n2`.

        1. Randomly generate two positive integers, `abs_n1` and `abs_n2`, between 1 and 100.
    2. Based on the value of `sign_combination`, assign signs to `n1` and `n2`.
    3. Calculate the final answer as `final_answer = n1 + n2`.
    4. Ensure that `final_answer` is within the range -200 to 200.

        - Confirm that two non-zero integers are generated.
    - Verify that `sign_combination` has randomness and covers all defined combinations.
    - Check if the addition operation is correctly calculated.
    - Ensure that the final answer is within the specified range.
    - Confirm that both operands meet the minimum complexity requirements (non-zero).

        - **Question Display**: Use LaTeX formatting to display the question, ensuring negative numbers are enclosed in parentheses.
    - **Answer Display**: Present the answer as a plain string without any LaTeX formatting.

        - The template focuses on generating addition problems with two non-zero integers and considers different sign combinations for diversity.
    - `fmt_num` is used to format numbers, automatically adding brackets around negative numbers.

        - Variations in absolute value size (1 to 100).
    - Different sign combinations: positive-positive, negative-negative, positive-negative, negative-positive.
    - Checks are included to avoid zero as an operand and ensure the result remains within a reasonable range.

        - After generation, verify that `n1` and `n2` are non-zero integers within the specified range.
    - Confirm that the final answer is an integer within the defined range.
    - Ensure that the sign combinations match the intended values.
    - Validate that the formatted question string adheres to LaTeX standards with negative numbers correctly bracketed.
