...
# Performance: 29.55s | Tokens: In=5796, Out=899
# Created At: 2026-01-27 21:05:34
# Fix Status: [Repaired] | Fixes: Regex=31, AST=8
...
def generate(level=1, **kwargs):
    ...
    for _safety_loop_var in range(1000):
        ...
        if abs(intermediate_A) > 100:
            continue
        factors_intermediate_A = [i for i in range(-30, 31) if i != 0 and intermediate_A % i == 0]
        op2 = safe_choice(['+', '-', '*', '/'])
        while op2 == '/' and len(factors_intermediate_A) == 0:
            n1 = random.randint(-30, 30)
            n2 = random.randint(-30, 30)
            op1 = safe_choice(['+', '-'])
            intermediate_A = safe_eval(f'{n1} {op1} {n2}')
            if abs(intermediate_A) > 100:
                continue
            factors_intermediate_A = [i for i in range(-30, 31) if i != 0 and intermediate_A % i == 0]
        n3 = safe_choice(factors_intermediate_A)
        intermediate_B = safe_eval(f'{intermediate_A} {op2} {n3}')
        if abs(intermediate_B) > 100:
            continue
        factors_intermediate_B = [i for i in range(-30, 31) if i != 0 and intermediate_B % i == 0]
        op3 = safe_choice(['+', '-', '*', '/'])
        while (op2 == '/' or op3 == '/') and len(factors_intermediate_B) == 0:
            n1 = random.randint(-30, 30)
            n2 = random.randint(-30, 30)
            op1 = safe_choice(['+', '-'])
            intermediate_A = safe_eval(f'{n1} {op1} {n2}')
            ...
            if abs(intermediate_B) > 100:
                continue
            factors_intermediate_B = [i for i in range(-30, 31) if i != 0 and intermediate_B % i == 0]
        n4 = safe_choice(factors_intermediate_B)
        final_answer = safe_eval(f'{intermediate_B} {op3} {n4}')
        if abs(final_answer) > 200:
            continue
        if not any((n < 0 for n in [n1, n2, n3, n4])):
            continue
        break
