...
# Performance: 20.85s | Tokens: In=6206, Out=712
# Created At: 2026-01-28 10:27:13
# Fix Status: [Repaired] | Fixes: Regex=23, AST=4
...
def generate(level=1, **kwargs):
    ...
    for _safety_loop_var in range(1000):
        op1 = safe_choice(['+', '-', '*'])
        op2 = safe_choice(operators)
        op3 = safe_choice(operators)
        if '*' in [op1, op2, op3] or '/' in [op1, op2, op3]:
            break
    ...
    if all((x > 0 for x in [n1, n2, n3, n4])):
        safe_choice([n1, n2, n3, n4])[0] *= -1
    temp1 = safe_eval(f'{n1} {op1} {n2}')
    if abs(temp1) > 100:
        continue
    if op2 == '/':
        while n3 != 0 and temp1 % n3 != 0:
            factors = [i for i in range(-10, 11) if i != 0 and temp1 % i == 0]
            if not factors:
                continue
            n3 = safe_choice(factors)
    temp2 = safe_eval(f'{temp1} {op2} {n3}')
    if abs(temp2) > 100:
        continue
    if op3 == '/':
        while n4 != 0 and temp2 % n4 != 0:
            factors = [i for i in range(-10, 11) if i != 0 and temp2 % i == 0]
            if not factors:
                continue
            n4 = safe_choice(factors)
