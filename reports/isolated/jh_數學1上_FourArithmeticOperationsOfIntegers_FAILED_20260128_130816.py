...
# Performance: 18.22s | Tokens: In=3112, Out=662
# Created At: 2026-01-28 13:08:16
...
def generate(level=1, **kwargs):
    This is a detailed specification for generating arithmetic problems with specific constraints and requirements. The goal is to create math questions that are suitable for K-12 students while ensuring they meet certain criteria in terms of complexity, range of numbers, and the inclusion of negative numbers.

    
        The domain specified here is "arithmetic," which indicates that the problems will involve basic arithmetic operations such as addition, subtraction, multiplication, and division.

        - **n1, n2**: Integers ranging from -20 to 20, non-zero.
    - **n3, n4**: Integers ranging from -10 to 10, non-zero (especially when used as divisors).
    - **op1, op2, op3**: Operators chosen from `['+', '-', '*', '/']`.

        - All intermediate values and the final answer must be integers.
    - Division operations must result in integer division.
    - The absolute value of `safe_eval(f"{ n1 } { op1 } { n2 }")` should not exceed 100.
    - The absolute value of `safe_eval(f"{ temp1 } { op2 } { n3 }")` should not exceed 100.
    - The absolute value of the final answer should not exceed 200.

        The template `multi_operation_integer_with_parentheses` specifies how to generate a problem with four operands and three operators, ensuring that at least one operator is multiplication or division. It also ensures that there is at least one negative number in the problem.

        - Ensure all constraints are met.
    - Implement logic for generating valid numbers and operators.
    - Handle edge cases such as integer division and range checks.
    - Format the question and answer appropriately using LaTeX for display.

        - Use `fmt_num(num)` to format each operand, handling negative numbers with parentheses.
    - Map operators to their LaTeX equivalents (e.g., `*` to `\times`, `/` to `\div`).
    - Assemble the expression string following the structure `safe_eval(f"{ n1 } { op1 } { n2 }") op2 n3 op3 n4`.
    - Use `clean_latex_output(q_str)` to wrap the final expression in LaTeX math mode.

        - The logic includes mechanisms for ensuring integer division by selecting appropriate divisors.
    - `op1` is limited to `+`, `-`, and `*` to simplify initial calculations.
    - Number ranges are chosen to keep intermediate results manageable for students.

        - Random variation in operand values and signs within specified ranges.
    - Random combination of operators ensuring at least one multiplication or division.
    - Maintaining complexity through the inclusion of negative numbers and appropriate operator combinations.
    - Ensuring all divisions result in integers by adjusting operands as needed.
    - Fixed structure with parentheses to maintain consistent presentation.

        After generation, verify that:
            - All operands meet their respective constraints.
    - Operators are within specified ranges and include at least one multiplication or division.
    - There is at least one negative number.
    - Intermediate results stay within specified bounds.
    - Divisions result in integers.
    - The final answer is an integer within the allowed range.

    This specification provides a comprehensive framework for generating arithmetic problems that meet educational standards while ensuring they are challenging yet solvable by K-12 students.
