...
# Performance: 30.58s | Tokens: In=4653, Out=998
# Created At: 2026-01-27 18:11:09
# Fix Status: [Repaired] | Fixes: Regex=25, AST=6
...
def generate(level=1, **kwargs):
    ...
    operand_forms = [safe_choice(['fraction', 'decimal', 'mixed_number', 'integer']) for _ in range(num_operands)]
    operators = [safe_choice(['+', '-', '*', '/']) for _ in range(num_operands - 1)]
    parentheses_structure = safe_choice(['none', 'left_group', 'right_group', 'full_group'])
    negative_numbers_count = random.randint(1, 2)
    operands = []
    for form in operand_forms:
        if form == 'fraction':
            ...
            operand = round(random.uniform(-100, 100), 2)
        elif form == 'mixed_number':
            whole = random.randint(-50, 50)
            numerator = random.randint(1, 9)
            denominator = random.randint(2, 10)
            operand = Fraction(numerator + whole * denominator, denominator)
        elif form == 'integer':
            operand = random.randint(-100, 100)
        operands.append(Fraction(operand))
    for _ in range(negative_numbers_count):
        index = random.randint(0, num_operands - 1)
        operands[index] *= -1
    if parentheses_structure == 'left_group':
        q = f'({fmt_num(operands[0])} {op_latex[operators[0]]} {fmt_num(operands[1])})'
        result = safe_eval(f'{operands[0]} {operators[0]} {operands[1]}')
        for i in range(2, num_operands):
            q += f' {op_latex[operators[i - 1]]} {fmt_num(operands[i])}'
            result = safe_eval(f'{result} {operators[i - 1]} {operands[i]}')
    elif parentheses_structure == 'right_group':
        if num_operands != 3:
            raise ValueError('Right group structure only applicable for 3 operands.')
        q = f'{fmt_num(operands[0])} {op_latex[operators[0]]} ({fmt_num(operands[1])} {op_latex[operators[1]]} {fmt_num(operands[2])})'
        result = safe_eval(f'{operands[0]} {operators[0]} ({operands[1]} {operators[1]} {operands[2]})')
    elif parentheses_structure == 'full_group':
        if num_operands != 4:
            raise ValueError('Full group structure only applicable for 4 operands.')
        q = f'({fmt_num(operands[0])} {op_latex[operators[0]]} {fmt_num(operands[1])}) {op_latex[operators[1]]} ({fmt_num(operands[2])} {op_latex[operators[2]]} {fmt_num(operands[3])})'
        result = safe_eval(f'({operands[0]} {operators[0]} {operands[1]}) {operators[1]} ({operands[2]} {operators[2]} {operands[3]})')
    else:
        q = f'{fmt_num(operands[0])} {op_latex[operators[0]]} {fmt_num(operands[1])}'
        result = safe_eval(f'{operands[0]} {operators[0]} {operands[1]}')
        for i in range(2, num_operands):
