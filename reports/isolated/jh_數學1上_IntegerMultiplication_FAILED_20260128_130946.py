...
# Performance: 26.57s | Tokens: In=1736, Out=1062
# Created At: 2026-01-28 13:09:46
...
def generate(level=1, **kwargs):
    This domain definition outlines a structured approach to generating arithmetic problems focusing on integer multiplication. The primary goal is to create questions that cover all possible combinations of positive and negative integers within specified ranges, ensuring educational value by challenging students with various scenarios.

    
    1. **Entities**:
       - `factor1` and `factor2`: These are the two integers involved in the multiplication operation.
         - Constraints: Both factors must be non-zero integers ranging from -10 to 10.
    ...
    3. **Constraints**:
       ...
       - Boundaries:
         - Integer Range:
           - Factors (factor1, factor2): -10 to 10
           - Result: -100 to 100 (since the maximum product of two factors within this range is 10 * 10 = 100 and the minimum is -10 * 10 = -100).
         - Computational Complexity: The absolute value of the result should not exceed three digits.
       - Exclusivity: None specified, meaning multiple constraints can apply simultaneously.
       - Minimum Complexity:
         - Must include a multiplication operation between two non-zero integers.
         - Must be capable of generating all four combinations of positive and negative signs (positive x positive, positive x negative, negative x positive, negative x negative).

    4. **Templates**:
       - `integer_multiplication_two_factors`: This template is designed to generate questions that meet the specified constraints.

    5. **Complexity Requirements**:
       - Generate two non-zero integers as multipliers and multiplicands.
       - Use multiplication operation.
       - Ensure generated problems cover all four sign combinations (positive x positive, positive x negative, negative x positive, negative x negative).
       - Each factor's absolute value must be at least 1.

    6. **Variables**:
       - `factor1_abs`: Absolute value of the first factor (randomly selected from 1 to 10).
       - `factor2_abs`: Absolute value of the second factor (randomly selected from 1 to 10).
       - `factor1_sign`: Sign of the first factor (True for positive, False for negative, randomly selected).
       - `factor2_sign`: Sign of the second factor (True for positive, False for negative, randomly selected).

    7. **Construction**:
       - Randomly generate `factor1_abs` and `factor2_abs`.
       - Randomly generate `factor1_sign` and `factor2_sign`.
       - Determine `factor1` based on its absolute value and sign.
       - Determine `factor2` based on its absolute value and sign.
       - Calculate the final answer using Python's multiplication operator (`*`).
       - Ensure the final answer is an integer within the expected range.

    8. **Implementation Checklist**:
       - Confirm two non-zero integers are generated as factors.
       - Verify each factor's absolute value is between 1 and 10.
       - Check if signs are correctly assigned based on boolean values.
       - Use Python’s multiplication operator to calculate results.
       - Ensure the problem covers all four sign combinations over time.
       - Confirm the final answer is an integer within the range of -100 to 100.

    9. **Formatting**:
       - Question Display: 
         - Start with a fixed Chinese narrative: "计算下列各式的值。"
         - Format each factor using `fmt_num(factor)`, which automatically adds parentheses for negative numbers.
         - Use LaTeX (`\\times`) for the multiplication operator and wrap the entire expression in `$ ... $`.
       - Answer Display:
         - Convert the final answer to a string directly.
         - Avoid using LaTeX or additional formatting functions.

    10. **Notes**:
        - The template focuses on basic integer multiplication concepts, particularly handling positive and negative signs.
        - `fmt_num` is crucial for maintaining clarity by automatically adding parentheses around negative numbers.

    
    - Factor Sign Combinations: Ensure all four combinations (positive x positive, positive x negative, negative x positive, negative x negative) are generated randomly.
    - Absolute Value Variations: Allow independent variation in the absolute values of both factors between 1 and 10.
    - Degradation Checks:
      - Avoid generating problems with zero as a factor to prevent trivial multiplication scenarios.
      - Ensure at least one factor has an absolute value greater than 1 to avoid overly simple multiplications.

    
    After generation, verify the following:
        - `factor1` and `factor2` are non-zero integers.
    - Both factors' absolute values are within the range of 1 to 10.
    - The final answer is an integer with an absolute value between -100 and 100.
    - The mathematical expression in the question has correct formatting, including proper handling of negative numbers with parentheses.
    - The answer display is a plain numeric string without additional formatting.

    This structured approach ensures that generated arithmetic problems are both educational and varied, providing students with comprehensive practice in integer multiplication.
