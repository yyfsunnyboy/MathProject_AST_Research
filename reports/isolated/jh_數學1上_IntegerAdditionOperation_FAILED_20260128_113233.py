...
# Performance: 26.46s | Tokens: In=1653, Out=1063
# Created At: 2026-01-28 11:32:33
...
def generate(level=1, **kwargs):
    op_latex = {'+': '+', '-': '-', '*': '\\times', '/': '\\div'}
    This is a detailed specification for generating arithmetic addition problems involving two integers. The domain focuses on creating integer addition questions with specific constraints and formatting rules to ensure educational value and consistency.

    
        - **n1**: An integer with the following constraints:
            - Value range: `-100` to `100`
      - Non-zero (to avoid trivial cases like n+0)
  
    - **n2**: An integer with the same constraints as `n1`.

        - The only operator used is addition (`+`).

        - **Calculability**: All intermediate values and the final answer must be precisely calculable using integer operations.
    - **Boundary**:
      - Integer range: `-100` to `100` for inputs `n1` and `n2`.
      - Result range: `-200` to `200` for the sum of `n1 + n2`.
    - **Minimum Complexity**:
      - Must have two operands.
      - Both operands (`n1` and `n2`) must be non-zero integers.

        - **Template Name**: `two_integer_addition`

        - Generate two non-zero integer operands (`n1`, `n2`).
    - Use the addition operator (`+`).
    - Ensure a diverse combination of operand signs (positive-positive, negative-negative, positive-negative, negative-positive).
    - The result must be within the specified range (-200 to 200).

        - **sign_combination**: A string randomly selected from `['pos_pos', 'neg_neg', 'pos_neg', 'neg_pos']` to control the sign combination of `n1` and `n2`.

        1. Randomly generate two positive integers `abs_n1` and `abs_n2`, each ranging from `1` to `100`.
    2. Convert `abs_n1` and `abs_n2` into `n1` and `n2` based on the value of `sign_combination`:
       - If `sign_combination` is 'pos_pos': `n1 = abs_n1`, `n2 = abs_n2`
       - If `sign_combination` is 'neg_neg': `n1 = -abs_n1`, `n2 = -abs_n2`
       - If `sign_combination` is 'pos_neg': `n1 = abs_n1`, `n2 = -abs_n2`
       - If `sign_combination` is 'neg_pos': `n1 = -abs_n1`, `n2 = abs_n2`
    3. Calculate the final answer: `final_answer = n1 + n2`.
    4. Check if `final_answer` is within the range `-200` to `200`. If not, regenerate `abs_n1` and `abs_n2` and repeat steps 2-4 until the condition is met.

        - Ensure two non-zero integers (`n1`, `n2`) are generated.
    - Confirm that `sign_combination` has randomness and covers all defined combinations.
    - Verify correct calculation of `n1 + n2`.
    - Check if `final_answer` is within the specified range.
    - Ensure all conditions for minimum complexity (non-zero operands) are met.

        - **Question Display**:
            - Use `fmt_num(n1)` and `fmt_num(n2)` to format numbers, adding parentheses around negative numbers.
      - Use `op_latex['+']` for the addition operator.
      - Concatenate formatted numbers and operators into a mathematical expression string.
      - Wrap the entire expression with `clean_latex_output()`.

    - **Answer Display**:
      - Convert `final_answer` directly to a string.
      - Avoid LaTeX formatting or use of `fmt_num()`.

        - The template focuses on generating non-zero integer addition problems, considering diverse sign combinations for operand variety.
    - `fmt_num` automatically handles negative number bracketing, aligning with K12 mathematical writing conventions.

        - **Variation Point 1**: Absolute value size of operands (1 to 100).
    - **Variation Point 2**: Sign combination of operands (positive-positive, negative-negative, positive-negative, negative-positive); these four combinations will be randomly selected.
    - **Degradation Checks**:
      - Avoid `0` as an operand through constraints on entities and minimum complexity requirements.
      - Ensure results are within a reasonable range to prevent overflow or excessively large values.

        - After generation, verify the following:
            - `n1` and `n2` are non-zero integers within `-100` to `100`.
      - `final_answer` is an integer within `-200` to `200`.
      - The sign combination of `n1` and `n2` matches the intent of the `sign_combination` variable.
      - The formatted question string adheres to LaTeX norms, with negative numbers correctly bracketed.
