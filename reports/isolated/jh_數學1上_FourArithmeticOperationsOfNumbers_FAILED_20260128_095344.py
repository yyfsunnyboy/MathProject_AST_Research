...
# Performance: 27.97s | Tokens: In=7359, Out=796
# Created At: 2026-01-28 09:53:44
...
def generate(level=1, **kwargs):
    ...
    for i in range(num_operands - 1):
        if i == 0:
            operand_type = 'rational'
        else:
            operand_type = 'rational' if random.random() < 0.5 else 'integer'

        if operand_type == 'rational':
            numerator = random.randint(-100, 100)
            denominator = random.randint(2, 20)            operand = Fraction(numerator, denominator)
        else:
            operand = random.randint(-100, 100)

        if i < num_operands - 2 and (operand.numerator > operand.denominator or operand < 0):
            operands.append(operand)
        elif len(operands) == 0:
            operands.append(operand)
        else:
            while True:
                new_operand = random.randint(-100, 100)
                if new_operand != operands[-1]:
                    operands.append(new_operand)
                    break

    for i in range(num_operands - 2):
        operator = safe_choice(['+', '-', '*', '/'])
        operators.append(operator)

    # Ensure at least one '*' or '/' and negative number
    while not any(op in ['*', '/'] for op in operators) or all(operand >= 0 for operand in operands):
        if not any(op in ['*', '/'] for op in operators):
            index = random.randint(0, num_operands - 2)
            operators[index] = '*' if random.random() < 0.5 else '/'
        if all(operand >= 0 for operand in operands):
        ...
        if operators[i] == '*' or operators[i] == '/':
            if operators[i] == '*':
                ...
                while operands[i + 1] == 0:
                    operands[i + 1] = random.randint(2, 20)
                current_value /= operands[i + 1]
        else:
            intermediate_result_list.append(current_value)
            intermediate_result_list.append(operators[i])
            current_value = operands[i + 1]
    intermediate_result_list.append(current_value)

    final_result = intermediate_result_list[0]
    for i in range(1, len(intermediate_result_list), 2):
        if intermediate_result_list[i] == '+':
            final_result += intermediate_result_list[i + 1]
        else:
            final_result -= intermediate_result_list[i + 1]
    ...
    question_str = " ".join(q_parts)
    question_str = clean_latex_output(question_str)

    # Step 4: Format answer as plain number
    a = str(final_result)
