...
# Performance: 23.60s | Tokens: In=1653, Out=979
# Created At: 2026-01-28 13:02:57
...
def generate(level=1, **kwargs):
    op_latex = {'+': '+', '-': '-', '*': '\\times', '/': '\\div'}
    This domain and template are designed for generating arithmetic addition problems involving two integers. Here's a breakdown of the key components:

    
        - **n1**: An integer with constraints that it must be within the range -100 to 100 and cannot be zero.
    - **n2**: Another integer with similar constraints as n1.
    ...
    - **邊界 (Boundaries)**:
      - Input integers `n1` and `n2` should be within the range of -100 to 100.
      - The result of `n1 + n2` should be within the range of -200 to 200.
    - **最小複雜度 (Minimum Complexity)**:
      - There must be two operands.
      - Both operands (`n1` and `n2`) must be non-zero integers.

        - **Name**: `two_integer_addition`
  
      **Complexity Requirements**:
          - Must generate two non-zero integer operands (`n1`, `n2`).
      - Must use the addition operator (`+`).
      - The sign combinations of `n1` and `n2` must be diverse (e.g., positive + positive, negative + negative, positive + negative, negative + positive).
      - The result must fall within the specified range (-200 to 200).

      **Variables**:
          - `sign_combination`: A string randomly selected from ['pos_pos', 'neg_neg', 'pos_neg', 'neg_pos'].

      **Construction Process**:
          1. Randomly generate two positive integers (`abs_n1` and `abs_n2`) within the range of 1 to 100.
      2. Convert `abs_n1` and `abs_n2` into `n1` and `n2` based on the value of `sign_combination`.
         - 'pos_pos': Both are positive.
         - 'neg_neg': Both are negative.
         - 'pos_neg': One is positive, the other is negative.
         - 'neg_pos': One is negative, the other is positive.
      3. Calculate the final answer: `final_answer = n1 + n2`.
      4. Ensure that `final_answer` falls within the range of -200 to 200. If not, regenerate `abs_n1` and `abs_n2`.

      **Implementation Checklist**:
          - Confirm that two non-zero integers (`n1`, `n2`) are generated.
      - Ensure that `sign_combination` is randomly selected and covers all defined combinations.
      - Verify the correct calculation of `n1 + n2`.
      - Check if `final_answer` falls within the specified range.
      - Confirm that all conditions for minimum complexity (non-zero integers) are met.

        - **Question Display**:
            - Use `fmt_num(n1)` and `fmt_num(n2)` to format numbers, adding parentheses around negative numbers.
      - Use `op_latex['+']` to represent the addition operator.
    ...
    - **Answer Display**:
      - Convert `final_answer` directly to a string without using LaTeX or `fmt_num()`.

        - This template focuses on generating addition problems with two non-zero integers, considering various combinations of positive and negative signs.
    - The `fmt_num` tool automatically handles the display of negative numbers in parentheses, aligning with K12 mathematical writing conventions.

        - **Variation Point 1**: The absolute value size of operands (1 to 100).
    - **Variation Point 2**: The sign combination of operands (positive + positive, negative + negative, positive + negative, negative + positive).

        - After generation, verify that:
            - `n1` and `n2` are non-zero integers within the range of -100 to 100.
      - `final_answer` is an integer within the range of -200 to 200.
      - The sign combinations of `n1` and `n2` match the intended `sign_combination`.
      - The formatted question string adheres to LaTeX norms, with negative numbers correctly bracketed.

    This structured approach ensures a wide variety of arithmetic addition problems while maintaining consistency and correctness.
