...
# Performance: 29.62s | Tokens: In=6178, Out=876
# Created At: 2026-01-27 21:24:27
# Fix Status: [Repaired] | Fixes: Regex=13, AST=0
...
def generate(level=1, **kwargs):
    ...
    operands = [random.randint(-20, 20) for _ in range(num_operands)]
    while any(op == 0 for op in operands):
        operands = [random.randint(-20, 20) for _ in range(num_operands)]
    
    operators = random.choices(['+', '-', '*', '/'], k=num_operators)
    while '*' not in operators and '/' not in operators:
        operators = random.choices(['+', '-', '*', '/'], k=num_operators)
    
    # Step 2: Ensure all divisions are exact
    for i, op in enumerate(operators):
        if op == '/':
            divisor = operands[i + 1]
            while divisor == 0 or abs(divisor) > 5 or (divisor == 1 or divisor == -1):
                divisor = random.randint(-20, 20)
    ...
    # Step 3: Construct the question LaTeX using fmt_num and op_latex
    bracket_type = safe_choice(['left_group', 'right_group', 'overall_group'])
    if num_operands == 4:
        bracket_type = safe_choice(['nested_left', 'nested_right'] + [bracket_type])
    ...
    if bracket_type == 'left_group':
        q = f"({q_parts[0]} {op_latex[operators[0]]} {q_parts[1]}) {op_latex[operators[1]]} {q_parts[2]}"
    elif bracket_type == 'right_group':
        q = f"{q_parts[0]} {op_latex[operators[0]]} ({q_parts[1]} {op_latex[operators[1]]} {q_parts[2]})"
    elif bracket_type == 'overall_group':
        q = f"({q_parts[0]} {op_latex[operators[0]]} {q_parts[1]} {op_latex[operators[1]]} {q_parts[2]})"
    elif bracket_type == 'nested_left':
        q = f"(({q_parts[0]} {op_latex[operators[0]]} {q_parts[1]}) {op_latex[operators[1]]} {q_parts[2]}) {op_latex[operators[2]]} {q_parts[3]}"
    elif bracket_type == 'nested_right':
        q = f"{q_parts[0]} {op_latex[operators[0]]} (({q_parts[1]} {op_latex[operators[1]]} {q_parts[2]}) {op_latex[operators[2]]} {q_parts[3]})"
    
    q = clean_latex_output(q)
    
    # Step 4: Format the answer as a plain number string
    result = operands[0]
    for i in range(num_operators):
        if operators[i] == '+':
            result += operands[i + 1]
