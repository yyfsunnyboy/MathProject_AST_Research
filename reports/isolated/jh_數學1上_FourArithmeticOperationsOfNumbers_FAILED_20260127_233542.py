...
# Performance: 29.91s | Tokens: In=6866, Out=1073
# Created At: 2026-01-27 23:35:42
# Fix Status: [Repaired] | Fixes: Regex=14, AST=6
...
def generate(level=1, **kwargs):
    ...
    for i in range(len(operators)):
        while operators[i] == '/' and operands[i + 1] == 0:
            idx = random.randint(0, num_operands - 2)
            if operators[idx] != '/':
                operators[idx] = gen_operator()
    ...
    if final_result.denominator == 1:
        a = str(final_result.numerator)
    elif abs(final_result.numerator) > final_result.denominator:
        whole = final_result.numerator // final_result.denominator
        rem = final_result.numerator % final_result.denominator
        a = f'{whole} {rem}/{final_result.denominator}'
    else:
        a = str(final_result)
