...
file_path = r'c:\Python\MathProject_AST_Research\core\code_generator.py'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Define the new function content
# Adapting user's logic to functional style and existing imports
new_function = r'''def auto_generate_skill_code(skill_id, queue=None, **kwargs):
    """
...
#==============================================================================\n\n'''
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write(header[1:] + final_code) # Hack to remove leading f from string repr above 
...
if not match:
    print("Could not find auto_generate_skill_code expression")
    exit(1)

start_index = match.start()

# Assume the function goes until the end of the file or next major definition
# In this specific file, it's the last function.
end_index = len(content)

# Replace
updated_content = content[:start_index] + new_function + "\n"

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(updated_content)

print(f"Successfully replaced auto_generate_skill_code.")
