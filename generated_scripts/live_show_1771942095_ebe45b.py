...
def generate(level=1, **kwargs):
    ...
    b = dividend - a * m
    part1_str = IntegerOps.format_latex(f'[({IntegerOps.fmt_num(a)} \\times {IntegerOps.fmt_num(m)}) + {IntegerOps.fmt_num(b)}] \\div {IntegerOps.fmt_num(divisor)}')
    part1_val = quotient
    d = rand_nz(-10, 15)
    e = rand_nz(-10, 10)
    f = rand_nz(1, 20)
    g = rand_nz(-10, 10)
    if level == 1:
        part2_str = IntegerOps.format_latex(f'|{IntegerOps.fmt_num(d)} \\times {IntegerOps.fmt_num(e)} - {IntegerOps.fmt_num(f)}|')
        part2_val = abs(d * e - f)
    else:
        part2_str = IntegerOps.format_latex(f'|{IntegerOps.fmt_num(d)} \\times {IntegerOps.fmt_num(e)} - {IntegerOps.fmt_num(f)} + {IntegerOps.fmt_num(g)}|')
        part2_val = abs(d * e - f + g)
    h = rand_nz(-10, 10)
    i = rand_nz(2, 5)
    j = rand_nz(1, 10)
    part3_str = IntegerOps.format_latex(f'({IntegerOps.fmt_num(h)} \\times {IntegerOps.fmt_num(i)} - {IntegerOps.fmt_num(j)})')
    k = rand_nz(-50, 50)
    if level == 1:
        ...
        question_text = IntegerOps.format_latex(f'計算 $$- {part1_str} + {part2_str} - {part3_str} + {IntegerOps.fmt_num(k)}$$ 的值。')
        ans = -part1_val + part2_val - part3_val + k
    return {'question_text': question_text, 'answer': '', 'correct_answer': IntegerOps.format_plain(int(ans)), 'mode': 1}
