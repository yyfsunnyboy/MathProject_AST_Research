...
def generate(level=1, **kwargs):
    ...
    part3_val = h * i - j
    if level == 1:
        question_text = IntegerOps.format_latex(f'\\text{{計算 }} $${part1_str} + {part2_str}$$ \\text{{ 的值。}}')
        ans = part1_val + part2_val
    elif level == 2:
        question_text = IntegerOps.format_latex(f'\\text{{計算 }} $${part1_str} - {part2_str} + {part3_str}$$ \\text{{ 的值。}}')
        ans = part1_val - part2_val + part3_val
    else:
        question_text = IntegerOps.format_latex(f'\\text{{計算 }} $$- {part1_str} + {part2_str} - {part3_str} + {fmt(k)}$$ \\text{{ 的值。}}')
