# 智學 Copilot 系統分析：前置技能自動建構系統 (Auto-Build Prerequisites)
...
* **Token 優化策略**：
    ...
    * *假設*：關係最密切的前置技能通常位於學習路徑的近期，而非遙遠的過去。
* **Context 注入 (Context Injection)**：
    * 從 `TextbookExample` 資料表撈取該技能的例題與詳解。
    * *目的*：讓 AI 不僅看技能名稱（如「配方法」），還能看到實際運算過程，進而準確判斷需要哪些基礎運算能力（如「完全平方公式」）。
* **輸出限制**：
